...
public static class Magic
{
	...
	public static unsafe Windows.UI.Color GetColorFromNameResourceId(int resourceId)
	{
		if (resourceId == 5114)
			return FromHsl(0, 0, 241.0 / 255.0); // Approximated high lightness
		else if (resourceId == 5118)
			return FromHsl(0, 0, 19.0 / 255.0); // Approximated low lightness
		else if (resourceId == 5115)
			return FromHsl(0, 21.0 / 255.0, 171.0 / 255.0); // Approximated low saturation, mid lightness
		else if (resourceId == 5116)
			return FromHsl(0, 21.0 / 255.0, 101.0 / 255.0); // Approximated low saturation, low lightness
		else if (resourceId == 5117)
			return FromHsl(0, 21.0 / 255.0, 99.0 / 255.0); // Approximated low saturation, very low lightness
		else
		{
			// Reverse mapping using hue and lightness/saturation limits
			for (int i = 0; i < 24; i++)
			{
				if (ColorNamesLight[i] == resourceId)
				{
					// Found in light category, construct color
					return FromHsl(
						(HueLimitsForSatLevel1[i] + HueLimitsForSatLevel1[i + 1]) / 2.0 / 255.0,
						1.0, // Assuming maximum saturation
						(LumLimitsForHueIndexHigh[i] + 1.0) / 255.0
					);
				}
				else if (ColorNamesMid[i] == resourceId)
				{
					// Found in mid category, construct color
					return FromHsl(
						(HueLimitsForSatLevel1[i] + HueLimitsForSatLevel1[i + 1]) / 2.0 / 255.0,
						1.0, // Assuming maximum saturation
						((LumLimitsForHueIndexHigh[i] + LumLimitsForHueIndexLow[i]) / 2.0) / 255.0
					);
				}
				else if (ColorNamesDark[i] == resourceId)
				{
					// Found in dark category, construct color
					return FromHsl(
						(HueLimitsForSatLevel1[i] + HueLimitsForSatLevel1[i + 1]) / 2.0 / 255.0,
						1.0, // Assuming maximum saturation
						(LumLimitsForHueIndexLow[i] - 1.0) / 255.0
					);
				}
			}
		}

		// Fallback color (black) if no match found
		return Windows.UI.Color.FromArgb(255, 0, 0, 0);
	}

	// Helper method to convert HSL to RGB and return as Windows.UI.Color
	...
	private static double GetColorComponent(double temp1, double temp2, double tempColor)
	{
		if (tempColor < 0.0)
			tempColor += 1.0;
		if (tempColor > 1.0)
		...
		if (6.0 * tempColor < 1.0)
			return temp1 + (temp2 - temp1) * 6.0 * tempColor;
		if (2.0 * tempColor < 1.0)
			return temp2;
		if (3.0 * tempColor < 2.0)
			return temp1 + (temp2 - temp1) * ((2.0 / 3.0) - tempColor) * 6.0;

		return temp1;
	}


	private unsafe static uint GetColorNameResourceId(Windows.UI.Color color)
	{
		HSL hsl = ConvertRgbToHsl(color);
		...
		if (s > 20.0)
		{
			ReadOnlySpan<int> hueLimits = default;
			...
			int hueIndex = 0;
			while (hueIndex < 23 && hueLimits[hueIndex] <= h) hueIndex++;
	...
	private static HSL ConvertRgbToHsl(Windows.UI.Color color)
	{
