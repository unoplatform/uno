...
# `adb install` returns before the app's own PACKAGE_ADDED broadcast is delivered, and on a freshly
# booted emulator the queues are still busy with boot broadcasts: it has landed up to 26s after
# `am start`, relaunching the Activity in the middle of the test run. Let the queues drain first.
sleep 5
if ! timeout 180 $ANDROID_HOME/platform-tools/adb shell am wait-for-broadcast-idle; then
	echo "##vso[task.logissue type=warning]The broadcast queues were not idle after 180s; starting the app anyway."
fi
