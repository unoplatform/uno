// Playwright capture helper for the Skia-WebAssembly damage-region harness.
//
// Loads the served Skia-WASM SamplesApp in a headless browser, scrolls the category list to generate a
// stream of damage-region updates (the whole viewport shifts without each item repainting), then captures
// two screenshots of the same settled state:
//   A) the damage-region-accumulated frame, and
...
// If damage-region rendering is correct, A and B are pixel-identical. The bash harness diffs them.
//
// Usage: node damage-region-wasm-capture.mjs <url> <renderer: webgl|software> <outA.png> <outB.png>
import { chromium } from 'playwright';

const [url, renderer, outA, outB] = process.argv.slice(2);
if (!url || !renderer || !outA || !outB) {
	console.error('Usage: node damage-region-wasm-capture.mjs <url> <webgl|software> <outA.png> <outB.png>');
	process.exit(2);
}
...
// Scroll the left category list to produce many damage-region frames.
