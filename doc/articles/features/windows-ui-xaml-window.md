...
Currently, `IsCustomizationSupported()` returns `true` on Desktop Windows (net11.0-desktop) and WinAppSDK (net11.0-windows10.0.x) targets.
...
> [!NOTE]
> This API is currently supported on Desktop Windows (net11.0-desktop) and WinAppSDK (net11.0-windows10.0.x) targets. On other platforms, the method exists but may have limited or no effect.
...
> [!NOTE]
> This API is currently supported on Desktop Windows (net11.0-desktop) and WinAppSDK (net11.0-windows10.0.x) targets. On other platforms, the property exists but may have limited or no effect.
...
> [!NOTE]
> `PreferredHeightOption` is supported on Desktop Windows (net11.0-desktop) and WinAppSDK (net11.0-windows10.0.x) targets when the title bar is extended into the content area. The `Height` property returns the value in actual pixels, not scaled points.
...
> * This API is supported on Desktop Windows (net11.0-desktop) and WinAppSDK (net11.0-windows10.0.x) targets.
