...
Uno Platform supports running applications using a common Skia Desktop shell, which is automatically used based on the running platform, using a single build output using the `net11.0-desktop` target framework from the [Uno.Sdk](xref:Uno.Features.Uno.Sdk).
...
1. Build your app with:
   ...
   ```dotnetcli
   dotnet publish -c Release -f net11.0-desktop
   ```
