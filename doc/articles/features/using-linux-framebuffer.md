...
- Launch the app using `dotnet run -f net11.0-desktop > logging.txt 2>&1`, then launch `tail -f logging.txt` in another terminal.
...
```dotnetcli
dotnet publish -c Release -f net11.0-desktop -r linux-x64 --self-contained true
```
