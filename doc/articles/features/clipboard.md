...
* To support pasting, read the clipboard in response to a paste gesture such as <kbd>Ctrl</kbd>+<kbd>V</kbd> (for example with a `KeyboardAccelerator`). This never prompts the user, and everything on the clipboard is available, including text, HTML, images (through `GetBitmapAsync`), and files copied from the OS file manager (through `GetStorageItemsAsync`). Large files are streamed as you read them, so they can be pasted cheaply. One caveat: a shortcut handler can run before the browser has delivered the pasted data, so the exact formats are not known yet at that moment and the common ones are reported as available. Probe the formats you care about and tolerate the `Get*Async` call throwing for a format the paste turned out not to carry.
