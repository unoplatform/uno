...
* Unlike plain text, HTML, and image copies, copying a custom format shows the browser's clipboard permission prompt, since browsers do not auto-grant writes of unsanitized data. If the user declines, the whole write is rejected, but the content still remains readable by your own app.
* Trigger copies from a user interaction such as a button click. Outside a user gesture the browser rejects the write, and while your app can still read the content back, other applications will not see it. The write is issued as soon as `SetContent` is called, and the data, including bitmaps and `SetDataProvider` results, follows as it becomes available, so preparing it asynchronously does not take the write outside the gesture.
* Reading back content your own app copied works without prompting the user, for as long as the clipboard is known not to have changed since: until the window loses and regains focus, or a copy or cut happens in the page. After that, reading goes back to the browser as described below.
* The `DataPackageView` returned by `GetContent` holds the content as it was at that moment, whether your own app wrote it or a paste gesture delivered it, so it can still be read after the clipboard changes.
* To support pasting, read the clipboard in response to a paste gesture such as <kbd>Ctrl</kbd>+<kbd>V</kbd> (for example with a `KeyboardAccelerator`). This never prompts the user, and everything on the clipboard is available, including text, HTML, images (through `GetBitmapAsync`), and files copied from the OS file manager (through `GetStorageItemsAsync`). A paste consisting of a single image, such as a screenshot, is exposed through `GetBitmapAsync`; pastes containing multiple files are exposed as files only. Large files are streamed as you read them, so they can be pasted cheaply. One caveat: a shortcut handler can run before the browser has delivered the pasted data, so the exact formats are not known yet at that moment and the common ones are reported as available. Probe the formats you care about and tolerate the `Get*Async` call throwing for a format the paste turned out not to carry.
...
* Outside a secure context (plain HTTP), the browser clipboard API is not available: `GetContent` then reports no formats, except for content your own app wrote or a paste gesture delivered.
