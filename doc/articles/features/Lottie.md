...
As Microsoft documented in there [Release Notes for Lottie-Windows v6.1.0](https://github.com/windows-toolkit/Lottie-Windows/releases/tag/v6.1.0), there's a new feature called _Theme property binding_. This allows to dynamically change a value (usually a color) at runtime.

To use this feature on Windows, you need to generate code with the [`LottieGen` tool](https://docs.microsoft.com/en-us/windows/communitytoolkit/animations/lottie-scenarios/getting_started_codegen). On Uno supported platforms, it's available at runtime using the `ThemableLottieVisualSource` instead of the classic `LottieAnimatedVisualSource`.

Here's **how to use this feature**:

1. Add a _css-like_ to your Lottie shape like this - put this in the name of the shape. That means the `nm` property in the json-generated file.
...
* Changing a property will force the player to reload its animation. Not only the animation will be restarted, but it can also big heavy on the CPU. Animation those values should be avoided.
