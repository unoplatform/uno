...
> [!NOTE]
> Uno Platform also supports building apps for using the `net11.0-desktop` target framework, using Uno Platform's own Skia Desktop implementation.
...
2. Build your app using the following command:
    ...
    ```pwsh
    msbuild /r /t:publish /p:TargetFramework=net11.0-windows10.0.26100 /p:Configuration=Release /p:Platform=x64 /p:PublishDir=c:\temp\myoutput
    ```
