...
Some files in the codebase use the `*.iOSmacOS.cs` suffix. This suffix predates the removal of the native macOS target and is now used for code shared between iOS and Mac Catalyst via Apple UIKit. These files are compiled when targeting `net11.0-ios` or `net11.0-maccatalyst`.
