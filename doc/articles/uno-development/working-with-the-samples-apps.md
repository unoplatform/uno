...
Samples are located in the [`UITests.Shared` folder](https://github.com/unoplatform/uno/tree/master/src/SamplesApp/UITests.Shared). UI-related samples are generally grouped by control, or by functional area for samples that aren't specific to a particular control (eg `VisualStateTests`). Non-UI samples are generally grouped by namespace of the tested feature.
...
1. Locate the folder corresponding to the control or class you want to create a sample for in `src/SamplesApp/UITests.Shared/`. The folder structure is typically `Namespace_In_Snake_Case/ControlNameTests`.
...
5. Format your XAML file: `dotnet xstyler -f src/SamplesApp/UITests.Shared/YourFolder/YourSample.xaml` (requires `dotnet tool restore` first, see [Code style](../contributing/guidelines/code-style.md#xaml-formatting-samplesapp)).
