...
| Skia            | `__UNO_SKIA__`     | Only available in the `net10.0-desktop` target framework, see [below](xref:Uno.Development.PlatformSpecificCSharp#webassembly-considerations) |
...
* `*.skia.cs` is built only for `net10.0-desktop`
* `*.reference.cs` is built only for `net10.0-desktop`
