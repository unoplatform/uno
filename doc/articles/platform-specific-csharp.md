...
When building an application that uses the Skia renderer, it is generally best to use such conditionals in order to build for only one target framework in class libraries (e.g., when only targeting `net11.0`, without a platform specifier like `net11.0-ios`).
...
| WebAssembly     | `__WASM__`         | Only available in the `net11.0-browserwasm` target framework, see [below](xref:Uno.Development.PlatformSpecificCSharp#webassembly-considerations) |
| Desktop         | `__DESKTOP__`      | Only available in the `net11.0-desktop` target framework. |
...
On `net11.0-windows10.0.xxxxx` target framework, an Uno Platform application isn't using Uno.UI at all. It's compiled using Microsoft's own tooling. For that reason, the `__UNO__` symbol is not defined on Windows. This aspect can optionally be leveraged to write code specifically intended for Uno.
...
* `*.wasm.cs` is built only for `net11.0-browserwasm`
* `*.browserwasm.cs` is built only for `net11.0-browserwasm`
* `*.desktop.cs` is built only for `net11.0-desktop`
* `*.iOS.cs` is built only for `net11.0-ios` and `net11.0-maccatalyst`
* `*.tvOS.cs` is built only for `net11.0-tvos`
* `*.UIKit.cs` is built only for `net11.0-ios` and `net11.0-maccatalyst` and `net11.0-tvos`
* `*.Apple.cs` is built only for `net11.0-ios` and `net11.0-maccatalyst` and `net11.0-tvos`
* `*.Android.cs` is built only for `net11.0-android`
* `*.WinAppSDK.cs` is built only for `net11.0-windows10` (eg. `net11.0-windows10.0.22621`)
