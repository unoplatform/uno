...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:PackageFormat=app
```

Where `{{RID}}` is either `osx-x64` or `osx-arm64`.

The resulting app bundle, which is a directory, will be located at `bin/Release/net11.0-desktop/{{RID}}/publish/{{APPNAME}}.app`.
...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:PackageFormat=app -p:CodesignKey={{identity}}
```
...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:PackageFormat=app -p:CodesignKey="Developer ID Application: John Appleby (XXXXXXXXXX)"
```

and

```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:PackageFormat=app -p:CodesignKey=A148697E815F6090DE9698F8E2602773296E2689
```
...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:PackageFormat=pkg -p:CodesignKey={{identity}} -p:PackageSigningKey={{installer_identity}}
```
...
The resulting installer will be located at `bin/Release/net11.0-desktop/{{RID}}/publish/{{APPNAME}}.pkg`.
...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:SelfContained=true -p:PackageFormat=dmg -p:CodesignKey={{identity}} -p:PackageSigningKey={{installer_identity}} -p:UnoMacOSNotarizeKeychainProfile={{notarytool-credentials}} -bl
```
...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:SelfContained=true -p:PackageFormat=dmg -p:CodesignKey={{identity}} -p:DiskImageSigningKey={{identity}}
```
...
The resulting disk image will be located at `bin/Release/net11.0-desktop/{{RID}}/publish/{{APPNAME}}.dmg`.
...
```bash
dotnet publish -f net11.0-desktop -r {{RID}} -p:SelfContained=true -p:PackageFormat=dmg -p:CodesignKey={{identity}} -p:DiskImageSigningKey={{identity}} -p:UnoMacOSNotarizeKeychainProfile={{notarytool-credentials}} -bl
```
