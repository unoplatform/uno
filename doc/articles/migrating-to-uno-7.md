...
11. Update assembly-qualified type names for the types relocated to `Uno.WinRT` — the assembly is now `Uno`.
12. Replace C# collection-initializer markup (`new StackPanel { child }`) with the explicit property — `Children` on a panel, `Child` on a `Border`, `Content` on a `ScrollViewer` — and enumerate `Children` rather than the element itself.
13. Fix the compile breaks from reduced visibility, retyped members, and rebased type hierarchies (see the two *WinUI parity* sections above).
