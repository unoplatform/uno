...
## Common Issues
The Uno Platform features and support is constantly evolving, but you may encounter some of the issues below while building your application. Some additional topics may be found in the [GitHub top discussions](https://github.com/unoplatform/uno/discussions).

#### .NET 7 Android Release AOT build fails with `The Java type `XXX` is generated by more than one managed type`

When building a net7.0-android app in release, you may get the following errors:
```
warning XA4214: The managed type `Microsoft.UI.Xaml.Controls.AnimatedIcon` exists in multiple assemblies: Uno.UI, Uno.UI, Uno.UI, Uno.UI.
error XA4215: The Java type `XXX` is generated by more than one managed type.
```

This is caused by [this .NET issue](https://github.com/xamarin/xamarin-android/issues/7473), that will be published as an updated 7.0.x update.

You can work around it in two ways.

##### Build an APK only for one CPU architecture
...
Note that this will create an APK that is only valid for the target that you selected. This is generally not a good solution to publish a build for the app store, as it restricts the number of compatible devices/

##### Downgrade to .NET 6
It's also possible to build using the .NET 6 SDK.

1. First, make sure to install the [.NET 6 SDK](https://dotnet.microsoft.com/en-us/download/dotnet/6.0).
1. Next to your solution file, create a `global.json` file with the following contents:
   ```json
   {
     "sdk": {
       "version": "6.0.401",
   ...
   ```
1. Update your `TargetFrameworks` list to use `net6.0-*` instead of `net7.0-*`
1. Once the file is created, open a command line to the folder containing the global.json file then run:
   ```shell
   dotnet workloads restore
   ```
