...
- In the debugger toolbar drop-down, select the `net11.0-desktop` target framework
...
- Set **Target framework** to `net11.0-desktop`
...
```shell
dotnet publish -f net11.0-desktop
```

If you wish to do a self-contained publish, run the following instead:

```shell
dotnet publish -f net11.0-desktop -r {{RID}} -p:SelfContained=true -p:TargetFrameworks=net11.0-desktop
```
...
> [!IMPORTANT]
> Due to changes in the .NET SDK, when providing an `{{RID}}` you will also need to add the following parameter `-p:TargetFrameworks=net11.0-desktop` for the publish command to succeed.
...
```shell
dotnet publish -f net11.0-desktop -r {{RID}} -p:SelfContained=true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:IncludeAllContentForSelfExtract=true
```
...
To publish your app that targets `net11.0-desktop` in your `.csproj`, you'll need to create a `.pubxml` file using Visual Studio, or use the file below:
...
<Project>
  <PropertyGroup>
    ...
    <TargetFramework>net11.0-desktop</TargetFramework>
    ...
    <PublishDir>bin\Release\net11.0-desktop\win-x64\app.publish\</PublishDir>
...
```shell
msbuild /m /r /target:Publish /p:Configuration=Release /p:PublishProfile="Properties\PublishProfiles\ClickOnceProfile.pubxml" /p:TargetFramework=net11.0-desktop
```
