...
1. Add NuGet package `Uno.Cupertino` to each of project heads:
    ...
    - > NU1605: Detected package downgrade: Uno.UI from 3.6.0-dev.275 to 3.5.1. Reference the package directly from the project to select a different version.
        ...
        solution: You need to update the version of `Uno.WinUI` (or `Uno.UI`) packages for all project heads that you are using to the higher version.
...
1. In `AppResources.xaml`, update the line that initializes the `CupertinoColors` to include the new palette override:    
    ```diff
    <Application.Resources>
		<ResourceDictionary>
			<ResourceDictionary.MergedDictionaries>
...
1. Add your custom font with the resource key `CupertinoFontFamily`:
    ...
    ```
1. In `AppResources.xaml`, add the line that initializes the `CupertinoFonts` to include the new font override:
    ```xml
    <Application.Resources>
    <ResourceDictionary>
        <ResourceDictionary.MergedDictionaries>
