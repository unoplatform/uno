...
namespace UITests.Shared.Windows_UI_Composition.DirtyRectangles
{
	...
	/// overlay) that the dirty region follows the actual rounded/curved shape rather than its bounding box.
	/// Manual-only — not in the byte-identity harness: on OpenGL/WebGL an antialiased curved edge rasterizes
	/// ~1px differently under a clipped (dirty-rect) present than under a full present, regardless of the
	...
	/// exact.
	/// </summary>
	[Sample("Windows.UI.Composition", Name = "DirtyRectangles_Rounded", IsManualTest = true,
		Description = "A rounded Border and an Ellipse repainting in place, for dirty-rectangles shape validation.")]
	public sealed partial class DirtyRectangles_Rounded : Page
	{
		private readonly DispatcherTimer _timer = new() { Interval = TimeSpan.FromMilliseconds(400) };
		private int _steps;

		public DirtyRectangles_Rounded()
		{
