	...
	public sealed partial class AutomationProperties_StructureAndProps : UserControl
	{
		...
		public AutomationProperties_StructureAndProps()
		{
			...
			AnnoTarget.SetValue(
				...
				new List<AutomationAnnotation>
				{
					new AutomationAnnotation { Type = AnnotationType.Comment, Element = AnnoComment },
					new AutomationAnnotation { Type = AnnotationType.GrammarError },
				});
		}
		...
		private TextBlock _reAddChild;

		private void OnRemoveReadd(object sender, Microsoft.UI.Xaml.RoutedEventArgs e)
		{
			if (_reAddChild is null)
			{
				// First click: add the target (fires a normal ChildAdded once flushed).
				_reAddChild = new TextBlock { Text = "Re-add target" };
				_reAddChild.SetValue(AutomationProperties.AutomationIdProperty, "struct-readd");
				DynamicContainer.Children.Add(_reAddChild);
			}
			else
			{
				// Subsequent clicks: remove then re-add synchronously (same coalescing window) —
				// WinUI cancels this pair, so no ChildRemoved/ChildAdded should reach the client.
				DynamicContainer.Children.Remove(_reAddChild);
				DynamicContainer.Children.Add(_reAddChild);
			}
		}
