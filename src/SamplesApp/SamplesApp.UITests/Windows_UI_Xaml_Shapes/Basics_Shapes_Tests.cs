...
namespace SamplesApp.UITests.Windows_UI_Xaml_Shapes
{
	public class Basics_Shapes_Tests : SampleControlUITestBase
	{
		...
		public void ValidateAllShapesBasicStrechesAlignemntsAndSize()
		{
			...
			//var id = _app.WaitForElement("_idInput").Single();
			//var run = _app.WaitForElement("_renderId").Single();
			// var renderArea = _app.WaitForElement("_root").Single();
			// var expectedSize = new Size(608, 688);
			...
			//	//"Max height large",
			//}
			//	.Select(name => Regex.Replace(name, @"([^\w]|[ ])(?<first>[a-z])", m => m.Groups["first"].Value.ToUpperInvariant()))
			//	.ToArray();

			//var tests = shapes.SelectMany(shape => stretches.SelectMany(stretch => sizes.Select(size => $"{shape}_{size}")));
			...
			var tolerance = new PixelTolerance().WithColor(15).WithOffset(3, 3);

			foreach (var test in _tests.Take(5))
			{
				try
				{
					...
					_app.EnterText("_id", test);
					_app.Tap("_renderId");

					var actual = TakeScreenshot(test, ignoreInSnapshotCompare: true);
					var testZone = _app.WaitForElement("_testZone").Single().Rect;

					var smallZone = new Rectangle(0, 0, 2, 2);
					var actualSmallZone = new Rectangle((int)testZone.X, (int)testZone.Y, 2, 2);
					//var actualRect = renderArea.Rect.ToRectangle();
					//var scale = (double)actualRect.Width / (double)expectedSize.Width;
					var scale = 2.0;

					ImageAssert.AreAlmostEqual(expected, ImageAssert.FirstQuadrant, actual, actualRect, scale, tolerance);
