...
public partial class EntryPoint : IDisposable
{
	...
	private async Task OnUpdateFileRequestedAsync(UpdateFileIdeMessage request)
	{
		try
		{
			if (request.FileContent is { Length: > 0 } fileContent)
			{
				...
				await ApplyFileContentAsync(filePath, fileContent, saveOpenDocument: request.ForceSaveOnDisk);
	...
	/// </summary>
	private async Task<Document?> ApplyFileContentAsync(string filePath, string fileContent, bool saveOpenDocument)
	{
	...
	private async Task OnUpdateFilesRequestedAsync(UpdateFilesIdeMessage request)
	{
		try
		{
			...
			{
				_ = Task.Run(() => WaitReadinessAndTriggerHotReloadAsync(createdFiles, deferredSaves, _ct.Token), _ct.Token);
			}
		}
		catch (Exception e) when (_ideChannelClient is not null)
		{
	...
	private async Task WaitReadinessAndTriggerHotReloadAsync(List<string> createdFiles, List<Document> deferredSaves, CancellationToken ct)
	{
		try
		{
			...
			foreach (var document in deferredSaves)
			{
				...
				{
					_debugAction?.Invoke($"Failed to save {document.FullName}: {e.Message}");
				}
			}

			// Programmatically trigger the "Apply Code Changes" command in Visual Studio,
			// which will trigger the hot reload (same mechanics as OnForceHotReloadRequestedAsync).
			_dte.ExecuteCommand("Debug.ApplyCodeChanges");
		}
		catch (Exception e)
		{
			_debugAction?.Invoke($"Batched hot-reload trigger failed: {e.Message}");
		}
	}
	...
	{
		if (await _asyncPackage.GetServiceAsync(typeof(SComponentModel)) is not IComponentModel componentModel
			|| componentModel.GetService<VisualStudioWorkspace>() is not { } workspace)
		{
			_debugAction?.Invoke("Roslyn workspace is not available; triggering hot reload without readiness wait.");
			return;
		}
		...
		if (stopwatch.Elapsed >= timeout)
		{
			_debugAction?.Invoke($"Workspace readiness timed out after {timeout}; triggering hot reload anyway.");
		}
	}
