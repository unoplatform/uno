...
/// <summary>
/// A SkiaSharp-free image decoder: parses the common encoded formats (PNG, GIF incl. animation, BMP, JPEG, WebP)
...
internal static partial class ManagedImageDecoder
{
	public static bool TryDecode(byte[] data, int? targetWidth, int? targetHeight, [NotNullWhen(true)] out DecodedImage? decoded)
	{
		...
		{
			// Malformed input is simply "not decoded" - there is no other codec behind this one.
			decoded = null;
			return false;
		}
	}
