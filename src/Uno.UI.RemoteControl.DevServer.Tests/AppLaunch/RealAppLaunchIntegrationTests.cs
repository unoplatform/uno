...
public class RealAppLaunchIntegrationTests : TelemetryTestBase
{
	...
	private async Task<string> BuildAppProjectAsync(SolutionHelper solution, int devServerPort)
	{
		...
		{
			FileName = "dotnet",
			Arguments = $"build \"{appProject}\" --configuration Debug --verbosity minimal --framework {_targetFramework}-desktop -p:UnoRemoteControlHost=localhost -p:UnoRemoteControlPort={devServerPort} /bl:LogFile=\"{solutionDir}\\build-app.binlog\"",
