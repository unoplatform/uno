...
public sealed class AppTaskContent
{
	...
	{
		AppTaskValidation.RequireAbsoluteUri(imageUri, nameof(imageUri));
		var isLocalFile = imageUri.IsFile &&
			!imageUri.IsUnc &&
			string.IsNullOrEmpty(imageUri.Host);
		var isCanonicalAppUri =
			imageUri.OriginalString.StartsWith("ms-appx:///", StringComparison.OrdinalIgnoreCase) ||
			imageUri.OriginalString.StartsWith("ms-appdata:///", StringComparison.OrdinalIgnoreCase);
	...
	/// </summary>
	/// <param name="assets">The assets generated by the task. The array must not be <c>null</c> or empty.</param>
	/// <returns>Task content that displays the specified generated assets.</returns>
	/// <remarks>
	/// Only the array itself is validated, matching Windows: <c>null</c> entries are accepted and dropped,
	/// so an array of only <c>null</c> entries produces content with no assets.
	/// </remarks>
	/// <exception cref="ArgumentException"><paramref name="assets"/> is <c>null</c> or empty.</exception>
	public static AppTaskContent CreateGeneratedAssetsResult(AppTaskResultAsset[] assets)
	{
		if (assets is null or { Length: 0 })
		{
			throw new ArgumentException("The generated asset array cannot be null or empty.", nameof(assets));
		}
		...
		return new(
			...
			generatedAssets: assets
				...
				.Select(static asset => asset.CreateSnapshot())
				.ToArray());
	}
