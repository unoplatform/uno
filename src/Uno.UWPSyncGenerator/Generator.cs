...
namespace Uno.UWPSyncGenerator
{
	abstract class Generator
	{
		...
		public virtual async Task Build(string baseName, string sourceAssembly)
		{
			...
			_iOSCompilation = await LoadProject($@"{topProject}.netcoremobile.csproj", "net10.0-ios26.0");
			_tvOSCompilation = await LoadProject($@"{topProject}.netcoremobile.csproj", "net10.0-tvos26.0");
			_androidCompilation = await LoadProject($@"{topProject}.netcoremobile.csproj", "net10.0-android");
			_unitTestsCompilation = await LoadProject($@"{topProject}.Tests.csproj", "net10.0");

			_netstdReferenceCompilation = await LoadProject($@"{topProject}.Reference.csproj", "net10.0");
			_wasmCompilation = await LoadProject($@"{topProject}.Wasm.csproj", "net10.0");
