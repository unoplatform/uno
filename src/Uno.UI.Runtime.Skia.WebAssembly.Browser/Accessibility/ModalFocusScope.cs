...
internal sealed partial class ModalFocusScope
{
	...
	internal void Activate(ModalFocusScope? parentScope)
	{
		...
		{
			this.Log().Debug($"Activate modal={_modalHandle} trigger={_triggerHandle} focusableChildren={_focusableChildren.Count} hasParent={parentScope is not null}");
		}
	...
	internal void UpdateChildren(List<IntPtr> focusableChildren)
	{
		if (this.Log().IsEnabled(LogLevel.Debug))
		{
			this.Log().Debug($"UpdateChildren modal={_modalHandle} count={focusableChildren.Count}");
		}
