...
internal partial class WebAssemblyAccessibility : SkiaAccessibilityBase
{
	...
	private void TryRegisterVirtualizedContainer(UIElement element)
	{
		...
		else if (element is ListViewBase listView)
		{
			...
			Action<SelectorItem, int> containerClearing = (itemElement, index) =>
			{
				if (region.OnItemUnrealized(itemElement.Visual.Handle, index))
				{
					RemoveFlowsFromTarget(itemElement.Visual.Handle);
					RemoveRelationshipSource(itemElement.Visual.Handle);
					QueueRelationshipRefresh(refreshResolved: true);
	...
	private sealed class VirtualizedRegionRegistration : IDisposable
	{
		...
		public void Dispose()
		{
			...
			try
			{
				...
				{
					Instance.CleanupVirtualizedHandle(handle);
				}
	...
	private static readonly object _enableAccessibilityRetryGate = new();
	...
	public static void EnableAccessibility()
	{
		...
		if (rootElement is null)
		{
			...
			lock (_enableAccessibilityRetryGate)
			{
					...
					_ =>
					{
						...
						NativeDispatcher.Main.Enqueue(() =>
						{
							...
							catch (Exception ex)
							{
								if (@this.Log().IsEnabled(LogLevel.Error))
								{
									@this.Log().Error($"[A11y] Accessibility retry failed: {ex.Message}", ex);
								}
								NativeMethods.OnAccessibilityActivationFailed();
							}
						});
		...
		try
		{
			AutomationProperties.RoleOverrideChangedCallback = @this.UpdateRoleOverride;
			AutomationProperties.FlowsFromChangedCallback = @this.RefreshFlowsFromTarget;
		...
		catch (Exception ex)
		{
			if (@this.Log().IsEnabled(LogLevel.Error))
			{
				@this.Log().Error($"[A11y] Accessibility activation failed: {ex.Message}", ex);
			}
			@this.ResetFailedAccessibilityActivation();
			return;
		}
		finally
		{
	...
	private void ResetFailedAccessibilityActivation()
	{
		...
		AutomationProperties.RoleOverrideChangedCallback = null;
		AutomationProperties.FlowsFromChangedCallback = null;
		FocusManager.SuppressNativeFocus = false;
		var focusSynchronizer = _focusSynchronizer;
		_focusSynchronizer = null;
		TryRollbackCleanup("focus synchronizer", () => focusSynchronizer?.Uninitialize());
		var liveRegionManager = _liveRegionManager;
		_liveRegionManager = null;
		TryRollbackCleanup("live regions", () => liveRegionManager?.ClearPending());
		...
		{
			TryRollbackCleanup("focus departure handler", () => focusManager.FocusObserver.FocusController.FocusDeparting -= OnFocusDeparting);
		}
		_focusSearchRoot = null;
		var modalScope = ActiveModalScope;
		ActiveModalScope = null;
		TryRollbackCleanup("modal focus scope", () => modalScope?.Deactivate());

		var virtualizedRegions = _virtualizedRegions.Values.ToArray();
		_virtualizedRegions.Clear();
		foreach (var registration in virtualizedRegions)
		{
			TryRollbackCleanup("virtualized region", registration.Dispose);
		}
		TryRollbackCleanup("ComboBox tracking", ResetComboBoxTracking);
		var modalDialogSubscriptions = _modalDialogSubscriptions.ToArray();
		_modalDialogSubscriptions.Clear();
		foreach (var subscription in modalDialogSubscriptions)
		{
			TryRollbackCleanup("modal dialog subscription", () =>
			{
				subscription.Key.Opened -= subscription.Value.Opened;
				subscription.Key.Closed -= subscription.Value.Closed;
			});
		}
		var dataGridRowSubscriptions = _dataGridRowSubscriptions.Values.ToArray();
		_dataGridRowSubscriptions.Clear();
		foreach (var subscription in dataGridRowSubscriptions)
		{
			TryRollbackCleanup("DataGrid row subscription", () => subscription.Element.DataContextChanged -= subscription.Handler);
			TryRollbackCleanup("DataGrid row binding", () => ReleaseDataGridRowBinding(subscription.Element));
		}
		_dataGridRealizedItems.Clear();
		var dataGridLayoutSubscriptions = _dataGridLayoutSubscriptions.Values.ToArray();
		_dataGridLayoutSubscriptions.Clear();
		foreach (var subscription in dataGridLayoutSubscriptions)
		{
			TryRollbackCleanup("DataGrid layout subscription", () => subscription.Element.LayoutUpdated -= subscription.Handler);
		}
		TryRollbackCleanup("DataGrid summary polling", ResetDataGridSummaryPolling);
		var dataGridFingerprintTimers = _dataGridFingerprintThrottleTimers.Values.ToArray();
		_dataGridFingerprintThrottleTimers.Clear();
		foreach (var timer in dataGridFingerprintTimers)
		{
			TryRollbackCleanup("DataGrid fingerprint timer", timer.Dispose);
		}
		TryRollbackCleanup("scroll subscriptions", ResetScrollSourceSubscriptions);
		...
		TryRollbackCleanup("semantic tree", NativeMethods.ClearSemanticTree);
		TryRollbackCleanup("focus sentinels", NativeMethods.RemoveFocusSentinels);
		TryRollbackCleanup("activation failure notification", NativeMethods.OnAccessibilityActivationFailed);
	}

	private void TryRollbackCleanup(string operation, Action cleanup)
	{
		try
		{
			cleanup();
		...
		catch (Exception ex)
		{
			if (this.Log().IsEnabled(LogLevel.Warning))
			{
				this.Log().Warn($"[A11y] Accessibility rollback could not complete {operation}: {ex.Message}");
			}
		}
	}
	...
	private void ExecuteAutomationAction(IntPtr handle, string action, Action<UIElement> execute)
	{
		try
		{
			if (TryGetLiveSemanticOwner(handle, out var owner) && IsActionAllowedByModalScope(owner, handle))
			{
				execute(owner);
			}
		}
		catch (Exception ex)
		{
			if (this.Log().IsEnabled(LogLevel.Warning))
			{
				this.Log().Warn($"[A11y] Ignored failed {action} action for handle={handle}: {ex.Message}");
			}
		}
	}

	private bool IsActionAllowedByModalScope(UIElement owner, IntPtr handle)
	{
		if (ActiveModalScope is not { IsActive: true } modalScope)
		{
			return true;
		}

		var modalHandle = modalScope.ModalHandle;
		if (handle == modalHandle || IsSemanticDescendantOf(handle, modalHandle))
		{
			return true;
		}

		if (owner is SelectorItem selectorItem && ItemsControl.ItemsControlFromItemContainer(selectorItem) is UIElement itemsOwner)
		{
			var ownerHandle = ResolveToSemanticHandle(itemsOwner);
			return ownerHandle == modalHandle || IsSemanticDescendantOf(ownerHandle, modalHandle);
		}
	...
	public static void OnSelection(IntPtr handle)
	{
		...
		@this.ExecuteAutomationAction(handle, "selection", owner =>
		{
			var peer = owner.GetOrCreateAutomationPeer();
			if (peer is not null && IsAutomationActionEnabled(peer) &&
				AriaMapper.GetPatternOrEventsSource(peer, PatternInterface.SelectionItem) is ISelectionItemProvider selectionItemProvider)
			{
	...
	private static string? ResolveDefaultSemanticRole(UIElement element, AutomationPeer? peer)
	{
		...
		if (landmarkType != AutomationLandmarkType.None)
		{
			...
			var name = peer is not null ? AriaMapper.ResolveLabel(peer) : AutomationProperties.GetName(element);
		...
		return role ?? (semanticType == SemanticElementType.Generic && !string.IsNullOrEmpty(AriaMapper.ResolveLabel(peer))
			? "group"
			: null);
	}
	...
	protected override void NotifyPropertyChangedEventCore(AutomationPeer peer, AutomationProperty automationProperty, object oldValue, object newValue)
	{
			...
			TryGetPeerOwner(peer, out element))
		{
			...
			TryGetPeerOwner(peer, out element))
		{
			NativeMethods.SetXamlAutomationId(element.Visual.Handle, peer.GetAutomationId() ?? string.Empty);
		}
		else if (automationProperty == AutomationElementIdentifiers.IsDialogProperty &&
			TryGetPeerOwner(peer, out element))
		{
			var roleOverride = NormalizeRoleOverrideForHost(element, peer, AutomationProperties.GetRoleOverride(element));
			var intrinsicRole = ResolveDefaultSemanticRole(element, peer);
			var presentedRole = roleOverride ?? intrinsicRole;
			NativeMethods.UpdateLandmarkRole(element.Visual.Handle, intrinsicRole ?? string.Empty);
			NativeMethods.UpdateAriaModal(element.Visual.Handle, peer.IsDialog());
			UpdateAuthoredRoleDescription(element, GetPrimaryRole(presentedRole), AriaMapper.ResolveLabel(peer));
		}
			...
			TryGetPeerOwner(peer, out element))
		{
			...
			NativeMethods.UpdateAriaLabel(element.Visual.Handle, AriaMapper.ResolveLabel(peer) ?? string.Empty);
		}
