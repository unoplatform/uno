...
internal class Win32NativeElementHostingExtension : ContentPresenter.INativeElementHostingExtension
{
	...
	{
		// Win32 attach must normalize hosted HWND style + parentage every time because templates can be swapped
		// while the native child is reused. We cannot assume previous attach state is still valid.
		...
		// Remove title bar and borders, then force a frame refresh so child-window style behavior is
		// applied immediately (activation/taskbar/state) before the hosted HWND is shown.
		PInvoke.SetWindowLong(hwnd, WINDOW_LONG_PTR_INDEX.GWL_STYLE, newStyleVal);
		var frameChangeSuccess = PInvoke.SetWindowPos(
			hwnd,
			HWND.Null,
			0,
			0,
		...
		if (!frameChangeSuccess)
		{
			this.LogError()?.Error($"{nameof(PInvoke.SetWindowPos)} failed after style update: {Win32Helper.GetErrorMessage()}");
		}

		this.LogTrace()?.Trace($"{nameof(AttachNativeElement)} child={hwnd.Value} host={Hwnd.Value} exStyle=0x{oldExStyleVal:X8}->0x{newExStyleVal:X8} style=0x{oldStyleVal:X8}->0x{newStyleVal:X8} frameChanged={frameChangeSuccess} active={PInvoke.GetActiveWindow().Value}");
	...
	public void DetachNativeElement(object content)
	{
		...
		_ = PInvoke.ShowWindow(hwnd, SHOW_WINDOW_CMD.SW_HIDE);
		// Detach in managed lifecycle terms only: keep the native HWND parented and just hide it.
		// Reparenting to HWND.Null can transiently promote child HWNDs to top-level windows between detach and dispose.
		// Visibility and input are controlled by SW_HIDE/SW_SHOWNORMAL + arrange, so parentage can remain stable.
	...
	public void ArrangeNativeElement(object content, Rect arrangeRect)
	{
		...
		// Arrange is where the hosted HWND receives final logical-to-rasterized bounds.
		// We delay first show until this point so WebView2 is never visible with stale default bounds.
