		...
		if (PInvoke.PeekMessage(out var msg, _hwnd, 0, 0, PEEK_MESSAGE_REMOVE_TYPE.PM_NOREMOVE) != 0)
		{
			if (msg.message == PInvoke.WM_CHAR)
			{
				...
				if (key != VirtualKey.Tab)
				{
					...
					// Filter control characters (0x00-0x1F, 0x7F) except CR and LF which are needed for Enter key.
					// Control characters are generated by Ctrl+letter combinations (e.g., Ctrl+F = 0x06) and
					// should not be inserted as text.
