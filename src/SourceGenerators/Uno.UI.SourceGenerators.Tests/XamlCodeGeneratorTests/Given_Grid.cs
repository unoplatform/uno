...
public class Given_Grid
{
	...
	public async Task When_Grid_Uses_Both_Syntaxes()
	{
		var test = new TestSetup(xamlFileName: "Grid_Uses_Both_Syntaxes.xaml", subFolder: Path.Combine("SourceGenerators", "Uno.UI.SourceGenerators.Tests", "XamlCodeGeneratorTests", "TestCases"))
		{
			ExpectedDiagnostics =
			{
				DiagnosticResult.CompilerError("XLS0501").WithSpan("//Project/0/Grid_Uses_Both_Syntaxes.xaml", 17, 33, 17, 33).WithArguments("The property 'ColumnDefinitions' is set more than once"),
				DiagnosticResult.CompilerError("XLS0501").WithSpan("//Project/0/Grid_Uses_Both_Syntaxes.xaml", 24, 30, 24, 30).WithArguments("The property 'RowDefinitions' is set more than once"),
			},
		};
