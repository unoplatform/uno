	...
	public class BindableTypeProvidersSourceGenerator : SourceGenerator
	{
					...
					using (writer.BlockInvariant(@"else"))
					{
						using (writer.BlockInvariant(@"lock(_knownMissingTypes)"))
						{
							using (writer.BlockInvariant(@"if(!_knownMissingTypes.Contains(type))"))
							{
								...
								writer.AppendLineInvariant(@"Debug.WriteLine($""The Bindable attribute is missing and the type [{{type.FullName}}] is not known by the MetadataProvider. Reflection was used the binding engine instead of the generated static metadata. Add the Bindable attribute to prevent this message and performance issues."");");
							}
						}
					}
					writer.AppendLineInvariant(@"#endif");
		...
		private void GenerateType(IndentedStringBuilder writer, INamedTypeSymbol ownerType)
		{
			...
			var fieldDependencyProperties =
				...
				where field.IsStatic
					&& field.Type == _dependencyPropertySymbol
				select field.Name;
		...
		private INamedTypeSymbol GetBaseType(INamedTypeSymbol type)
		{
			if (type.BaseType != null)
			{
				...
				var isKnownBaseType = type.BaseType == _objectSymbol
					|| type.BaseType == _javaObjectSymbol
					|| type.BaseType == _nsObjectSymbol;
		...
		private bool IsStringIndexer(IPropertySymbol property)
		{
			return property.IsIndexer
				...
				&& property.Parameters.Any(p => p.Type == _stringSymbol);
		}
