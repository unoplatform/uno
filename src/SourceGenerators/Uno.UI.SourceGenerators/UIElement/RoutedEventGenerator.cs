...
{
	[Generator]
	public class RoutedEventFlagsGenerator : ISourceGenerator
	{
		...
		public void Execute(GeneratorExecutionContext context)
		{
			...
			{
				var visitor = new RoutedEventFlagsVisitor(context);
				visitor.Visit(context.Compilation.SourceModule);
			}
		}

		private class RoutedEventFlagsVisitor : SymbolVisitor
		{
			...
			private readonly INamedTypeSymbol _pointerArgsSymbol;
			private readonly INamedTypeSymbol _tappedArgsSymbol;
			private readonly INamedTypeSymbol _doubleTappedArgsSymbol;
			private readonly INamedTypeSymbol _rightTappedArgsSymbol;
			private readonly INamedTypeSymbol _holdingArgsSymbol;
			private readonly INamedTypeSymbol _dragArgsSymbol;
			private readonly INamedTypeSymbol _keyArgsSymbol;
			private readonly INamedTypeSymbol _routedArgsSymbol;
			private readonly INamedTypeSymbol _manipStartingArgsSymbol;
			private readonly INamedTypeSymbol _manipStartedArgsSymbol;
			private readonly INamedTypeSymbol _manipDeltaArgsSymbol;
			private readonly INamedTypeSymbol _manipInertiaArgsSymbol;
			private readonly INamedTypeSymbol _manipCompletedArgsSymbol;

			public RoutedEventFlagsVisitor(GeneratorExecutionContext context)
			{
				_context = context;

				var comp = context.Compilation;

				_uiElementSymbol = comp.GetTypeByMetadataName(XamlConstants.Types.UIElement)
					?? throw new Exception("Unable to find " + XamlConstants.Types.UIElement);
			...
			private bool IsUIElement(INamedTypeSymbol typeSymbol)
			{
				while (typeSymbol != null && !SymbolEqualityComparer.Default.Equals(typeSymbol, _uiElementSymbol))
				{
					typeSymbol = typeSymbol.BaseType;
				}

				return SymbolEqualityComparer.Default.Equals(typeSymbol, _uiElementSymbol);
			}

			private void ProcessType(INamedTypeSymbol typeSymbol)
			{
				if (!IsUIElement(typeSymbol))
				{
					return;
				}
				...
				if (hasGeneratedProperties)
				{
					...
					builder.AppendLineInvariant("// This file has been generated by Uno.UI (RoutedEventGenerator)");
					builder.AppendLineInvariant("// ******************************************************************");
					builder.AppendLineInvariant("// </auto-generated>");
					builder.AppendLine();
