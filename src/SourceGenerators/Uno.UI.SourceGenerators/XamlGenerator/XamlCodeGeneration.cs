...
namespace Uno.UI.SourceGenerators.XamlGenerator
{
	...
	internal partial class XamlCodeGeneration
	{
		...
		private static readonly char[] _equalsArray = new[] { '=' };
		...
		private string[] _globalXamlClrNamespaces;
		...
		public XamlCodeGeneration(GeneratorExecutionContext context)
		{
			...
			_enableImplicitXamlNamespaces = bool.TryParse(
				context.GetMSBuildPropertyValue("UnoEnableImplicitXamlNamespaces"), out var implicitXamlNs) && implicitXamlNs;

			_globalXamlClrNamespaces = (context.GetMSBuildPropertyValue("UnoGlobalXamlClrNamespacesProperty") ?? "")
				.Split(_commaArray, StringSplitOptions.RemoveEmptyEntries);

			_implicitXamlPrefixes = (context.GetMSBuildPropertyValue("UnoImplicitXamlPrefixesProperty") ?? "")
				.Split(_commaArray, StringSplitOptions.RemoveEmptyEntries)
				.Select(entry => { var parts = entry.Split(_equalsArray, 2); return (parts[0], parts.Length > 1 ? parts[1] : ""); })
				.ToArray();
		...
		public List<KeyValuePair<string, SourceText>> Generate()
		{
			...
			try
			{
				...
				{
					var additionalClrNamespaces = ReadGlobalXmlnsDefinitionsFromCompilation(_generatorContext.Compilation);
					if (additionalClrNamespaces.Length > 0)
					{
						_globalXamlClrNamespaces = _globalXamlClrNamespaces.Concat(additionalClrNamespaces).Distinct().ToArray();
					}
				}

				// Parse XAML files
				var xamlParser = new XamlFileParser(
					_excludeXamlNamespaces,
					_includeXamlNamespaces,
					excludeXamlNamespaces,
					includeXamlNamespaces,
					_metadataHelper,
					_enableImplicitXamlNamespaces,
					_implicitXamlPrefixes);
				...
				var csharpFiles = (Debugger.IsAttached
					...
					.Select<XamlFileDefinition, (XamlFileDefinition definition, SourceText? code, IEnumerable<XamlGenerationException> errors)>(file =>
					{
						var generator = new XamlFileGenerator(
							...
							globalXamlClrNamespaces: _enableImplicitXamlNamespaces ? _globalXamlClrNamespaces : Array.Empty<string>()
						);
		...
		private static string[] ReadGlobalXmlnsDefinitionsFromCompilation(Compilation compilation)
		{
			var xmlnsDefinitionAttrName = "Windows.Foundation.Metadata.XmlnsDefinitionAttribute";
			var results = new List<string>();

			foreach (var attribute in compilation.Assembly.GetAttributes())
			{
				if (attribute.AttributeClass?.ToDisplayString() == xmlnsDefinitionAttrName
					&& attribute.ConstructorArguments.Length >= 2)
				{
					...
					if (xmlNamespace == XamlConstants.PresentationXamlXmlNamespace
						&& !string.IsNullOrEmpty(clrNamespace))
					{
						results.Add(clrNamespace!);
					}
				}
			}
