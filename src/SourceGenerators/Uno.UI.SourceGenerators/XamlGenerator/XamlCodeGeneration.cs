...
namespace Uno.UI.SourceGenerators.XamlGenerator
{
	...
	internal partial class XamlCodeGeneration
	{
		...
		/// <summary>
		/// Whether auto code-behind generation is enabled at project level. Default true.
		/// </summary>
		private readonly bool _generateCodeBehindEnabled;
		...
		/// Used by XamlFileGenerator to suppress warnings and adjust x:Bind resolution.
		/// </summary>
		internal ConcurrentDictionary<string, INamedTypeSymbol> AutoGeneratedCodeBehindFiles { get; } = new();
		...
		public XamlCodeGeneration(GeneratorExecutionContext context)
		{
			...
			if (!bool.TryParse(context.GetMSBuildPropertyValue("UnoEnableImplicitXamlNamespaces"), out _enableImplicitXamlNamespaces))
			{
				_enableImplicitXamlNamespaces = false;
			}

			_globalXamlNamespaceUri = context.GetMSBuildPropertyValue("UnoGlobalXamlNamespaceUri");
			if (string.IsNullOrEmpty(_globalXamlNamespaceUri))
			{
				_globalXamlNamespaceUri = XamlConstants.GlobalNamespaceUri;
			}
			...
			// Auto code-behind generation settings
			var generateCodeBehindValue = context.GetMSBuildPropertyValue("UnoGenerateCodeBehind");
		...
		public List<KeyValuePair<string, SourceText>> Generate()
		{
			...
			try
			{
				...
				// Auto-generate code-behind for XAML files missing a developer-authored class
				GenerateCodeBehindForMissingClasses(xamlFiles, outputFiles, ct);
		...
		private void GenerateCodeBehindForMissingClasses(
			XamlFileDefinition[] xamlFiles,
			List<KeyValuePair<string, SourceText>> outputFiles,
			CancellationToken ct)
		{
			foreach (var file in xamlFiles)
			{
				...
				// Check if the file has x:Class
				var classMember = topLevelControl.Members.FirstOrDefault(m => m.Member.Name == "Class");
				if (classMember?.Value is null)
				{
					// FR-006: No x:Class attribute - skip
					continue;
				}

				var xClassFullName = classMember.Value.ToString() ?? "";
				...
				// FR-005: Check if the class already exists in the compilation
				var existingSymbol = _metadataHelper.FindTypeByFullName(xClassFullName) as INamedTypeSymbol;
				...
				// Resolve the base type from the XAML root element
				var baseTypeSymbol = ResolveXamlRootElementType(topLevelControl.Type);
				if (baseTypeSymbol is null)
				{
					// Cannot determine the base type — skip generation to avoid emitting
					// a wrong base type that conflicts with the XAML compiler's output
					continue;
				}
				...
				// Track the auto-generated file so XamlFileGenerator can suppress warnings
				// and use the resolved base type for x:Bind resolution
				AutoGeneratedCodeBehindFiles.TryAdd(file.UniqueID, baseTypeSymbol);
			}
		}

		/// <summary>
		/// Resolves the base type symbol for a XAML root element.
		/// Uses the known namespace mappings to find the type in the compilation.
		/// </summary>
		private INamedTypeSymbol? ResolveXamlRootElementType(XamlRedirection.XamlType xamlType)
		{
			// Try known WinUI presentation namespaces first
			var trimmedNamespace = xamlType.PreferredXamlNamespace.Split('?').First();

			// Check if it's a standard WinUI namespace
			if (XamlConstants.Namespaces.PresentationNamespaces is { } presentationNamespaces
				&& (trimmedNamespace == XamlConstants.PresentationXamlXmlNamespace
					|| trimmedNamespace == XamlConstants.XamlXmlNamespace))
			{
				var namespacesToSearch = trimmedNamespace == XamlConstants.PresentationXamlXmlNamespace
					? XamlConstants.Namespaces.PresentationNamespaces
					: new[] { "System" };

				foreach (var clrNamespace in namespacesToSearch)
				{
					if (_metadataHelper.FindTypeByFullName(clrNamespace + "." + xamlType.Name) is INamedTypeSymbol result)
					{
						return result;
					}
				}
					...
					using (writer.BlockInvariant("internal static void RegisterResourceDictionariesBySourceLocal()"))
					{
						...
						foreach (var file in xamlFiles.Where(IsResourceDictionary))
						{
							...
							if (_enableAlcAppSupport)
							{
								...
								writer.AppendLineIndented($"global::Uno.UI.ResourceResolver.RegisterResourceDictionaryBySource(uri: \"{XamlFilePathHelper.MsResourceFilesPrefix}{sourceLink}\", context: {contextArg}, dictionary: () => {file.UniqueID}_ResourceDictionary, {filePath}, __alc);");
								// Local resources can also be found through the ms-appx:/// prefix
								writer.AppendLineIndented($"global::Uno.UI.ResourceResolver.RegisterResourceDictionaryBySource(uri: \"{XamlFilePathHelper.AppXIdentifier}{sourceLink}\", context: {contextArg}, dictionary: () => {file.UniqueID}_ResourceDictionary, null, __alc);");
							}
							else
							{
								// Standard registration without ALC support
								writer.AppendLineIndented($"global::Uno.UI.ResourceResolver.RegisterResourceDictionaryBySource(uri: \"{XamlFilePathHelper.MsResourceFilesPrefix}{sourceLink}\", context: {contextArg}, dictionary: () => {file.UniqueID}_ResourceDictionary, {filePath});");
								// Local resources can also be found through the ms-appx:/// prefix
