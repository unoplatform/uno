...
#pragma warning restore 649 // Unused member
		...
		public XamlCodeGeneration(Compilation sourceCompilation, ProjectInstance msbProject, Project roslynProject)
		{
			...
			_projectFullPath = msbProject.GetProperty("MSBuildProjectFullPath")?.EvaluatedValue;
		...
		private string GetSourceLink(ProjectItemInstance projectItemInstance)
		{
			...
			if (link.IsNullOrEmpty())
			{
				if (definingProjectFullPath.IsNullOrEmpty())
				{
					...
					if (link.IsNullOrEmpty())
					{
						return Path.IsPathRooted(projectItemInstance.EvaluatedInclude)
							? projectItemInstance.EvaluatedInclude.TrimStart(_projectDirectory).TrimStart(Path.DirectorySeparatorChar)
							: projectItemInstance.EvaluatedInclude;
					}
				}
				else
				{
