	...
	internal partial class XamlCodeGeneration
	{
			...
			CancellationToken ct)
		{
			foreach (var file in xamlFiles)
			{
				...
				if (!ShouldGenerateCodeBehind(perFileValue, _generateCodeBehindEnabled ? "true" : "false"))
				{
					continue;
				}
				...
				var sourceText = EmitCodeBehind(ns, className, baseTypeFullName);
				var hintName = $"{xClassFullName}.codebehind.g.cs";
				...
				var trackingSymbol = baseTypeSymbol
					?? _metadataHelper.FindTypeByFullName("Microsoft.UI.Xaml.Controls.Page") as INamedTypeSymbol;
				if (trackingSymbol is not null)
				{
					AutoGeneratedCodeBehindFiles.TryAdd(file.UniqueID, trackingSymbol);
				}
			}
		}
		...
		/// </summary>
		private static bool ShouldGenerateCodeBehind(string? perFileValue, string? globalValue)
		{
		...
		/// </summary>
		private static string EmitCodeBehind(string ns, string className, string baseTypeFullName)
		{
...
namespace {ns}
{{
	partial class {className} : {baseTypeFullName}
	{{
		public {className}()
		{{
