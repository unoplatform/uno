...
{
	private static readonly DiagnosticDescriptor MissingEffectiveViewportTemplate = new(
		"UNOMIXIN0001",
		"Missing EffectiveViewport template",
		"The FrameworkElement.EffectiveViewport.cs AdditionalFile is missing or unreadable. EffectiveViewport partials will not be generated for Android.",
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		// Output is wrapped in #if UNO_MIXIN_GENERATION && __ANDROID__ so it compiles only on Android.
		...
		context.RegisterSourceOutput(combined, static (ctx, data) =>
		{
			...
			{
				ctx.ReportDiagnostic(Diagnostic.Create(MissingEffectiveViewportTemplate, Location.None));
			}
		});
	}
	...
	public static string GenerateFrameworkElementMixins()
	{
