...
/// managed <see cref="TextBoxCore"/>. Shared by both invisible proxies, which derive from
...
internal sealed class InvisibleTextBoxFloatingCursor
{
	...
	{
		_origin = point;
		IsActive = extension?.ProcessCaretDragGesture(TextBoxCore.CaretDragPhase.Begin, default) ?? false;
	}
	...
	public void Update(InvisibleTextBoxViewExtension? extension, CGPoint point)
	{
		...
		IsActive = extension?.ProcessCaretDragGesture(TextBoxCore.CaretDragPhase.Update, offset) ?? false;
	}
