...
internal static partial class AndroidResourceNameEncoder
{
	...
	public static string EncodeFileSystemPath(string path, string prefix = "Assets")
		...
		=> global::System.IO.Path.Combine(prefix, EncodePath(path, global::System.IO.Path.DirectorySeparatorChar, false));

	public static string EncodeResourcePath(string path, bool isDrawable)
		=> EncodePath(path, '/', isDrawable);

	public static string EncodeDrawablePath(string path)
		=> EncodeResourcePath(path, true).Replace('/', '_');

	private static string EncodePath(string path, char separator, bool isDrawable)
	{
		...
		var fileName = isDrawable ?
			global::System.IO.Path.GetFileName(alignedPath) :
			global::System.IO.Path.GetFileNameWithoutExtension(alignedPath);
