...
namespace Windows.Storage
{
	public partial class FileIO
	{
		...
		private static async Task AppendWriteTextAsync(IStorageFile file, string contents, Streams.UnicodeEncoding encoding, bool append)
		{
			...
			Text.Encoding encodingForWriter = Text.Encoding.UTF8;
			switch (encoding)
			{
				case Streams.UnicodeEncoding.Utf8:
					encodingForWriter = Text.Encoding.UTF8;
					break;
				case Streams.UnicodeEncoding.Utf16LE:
					encodingForWriter = Text.Encoding.Unicode;
					break;
				case Streams.UnicodeEncoding.Utf16BE:
					encodingForWriter = Text.Encoding.BigEndianUnicode;
					break;
			}
		...
		private static IAsyncAction AppendWriteTextAsync( IStorageFile file,  string contents, bool append)
		{
		...
		public static IAsyncAction WriteTextAsync( IStorageFile file,  string contents) => AppendWriteTextAsync(file, contents, false);

		public static IAsyncAction AppendTextAsync( IStorageFile file,  string contents) => AppendWriteTextAsync(file, contents, true);
	}
}
