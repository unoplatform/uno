...
namespace Windows.Globalization.NumberFormatting
{
    internal static class MathExtensions
    {
		...
		public static double MultiplyByPow10(this double value, int pow10)
		{
			...
			var indexOfSeperator = text.IndexOf(numberDecimalSeparator);

			if (indexOfSeperator == -1)
			{
				var part = new string(Enumerable.Repeat('0', pow10).ToArray());
				return double.Parse($"{text}{part}");
			}
			...
			if (diff <= 0)
			{
				...
				return double.Parse($"{text.Remove(indexOfSeperator, numberDecimalSeparator.Length)}{part}");
			}
			...
			return double.Parse($"{integerPart}{integerNewPart}{numberDecimalSeparator}{newFractionPart}");
		}
    }
}
