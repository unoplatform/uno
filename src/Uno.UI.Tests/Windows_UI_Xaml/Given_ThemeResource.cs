	...
	public class Given_ThemeResource
	{
		...
		public void When_Theme_Changed_ResourceKey()
		{
			using (UseFluentResources())
			{
				...
				// Dark text
				Assert.IsLessThan(((SolidColorBrush)textBlock.Foreground).Color.R, 100);

				using var _ = SwapSystemTheme();

				// Light text
				Assert.IsGreaterThan(((SolidColorBrush)textBlock.Foreground).Color.R, 200);
			}
		}
		...
		public void When_Theme_Changed_ImplicitStyle()
		{
			using (UseFluentResources())
			{
				...
				// Dark text
				Assert.IsLessThan(((SolidColorBrush)button.Foreground).Color.R, 100);

				using var _ = SwapSystemTheme();

				// Light text
				Assert.IsGreaterThan(((SolidColorBrush)button.Foreground).Color.R, 200);
			}
		}
