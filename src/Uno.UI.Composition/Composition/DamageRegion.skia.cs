...
internal sealed class DamageRegion : IDisposable
{
	...
	private readonly SKPath _rects = new() { FillType = SKPathFillType.Winding };
	private readonly SKPath _exact = new();
	private readonly SKPath _clamp = new();
	...
	internal void UnionRect(SKRect rect)
	{
		...
		// the bookkeeping below, where SKRect.Union drops it while SKPath.AddRect would normalize and
	...
	internal void SnapshotAndReset(SKPath destination, SKRect clampTo)
	{
		...
		// A throw between here and Reset() would leave the region claiming rects it no longer holds, so
		// reset unconditionally to keep the next frame sane.
		try
		{
			if (_rectCount > 0)
			{
				_rects.AddRect(_open);
				_rects.Transform(SKMatrix.Identity, destination);
