...
partial class MetadataAPI
{
	//public static CDependencyProperty TryGetPropertyByName(
	//	CClassInfo pType,
	//	string strName,
	//	bool allowDirectives)
	//{
	//	CDependencyProperty ppProperty = null;

	//	if (IsKnownIndex(pType.GetIndex()))
	//	{
	//		// Built-in DPs also act as their own property wrappers.

	//		const CDependencyProperty dp = null;
	...
	//		if (dp != null && !dp.Is<CCustomDependencyProperty>())
	//		{
	...
	//		TryImportPropertyInfoFromMetadataProvider(pType, strName, ppProperty);
	...
	//		CClassInfo pBaseType = pType.GetBaseType();

	//		if (pBaseType.GetIndex() != KnownTypeIndex.UnknownType)
	//		{
	//			TryGetPropertyByName(pBaseType, strName, ppProperty, allowDirectives);

	//			if (ppProperty != null)
	//			{
	//				// Store a reference to the inherited property, so we don't need to traverse our base types to find it in the future.
	//				AssociateProperty(pType, ppProperty);
	//			}
	//		}
	//	}
	//}

	//private static CDependencyProperty TryImportPropertyInfoFromMetadataProvider(CClassInfo pType, xstring_ptr_view& strName)
	//{
	//}
}
