...
internal partial class DXamlCore
{
	...
	public static bool IsInitializedStatic => Current is { } current && current.IsInitialized;
	...
	internal void EnsureCoreApplicationInitialized()
	{
		//	if (!m_pDOCoreApp)
		//	{
	...
	internal void ConfigureJupiterWindow(CoreWindow? pCoreWindow)
	{
		if (_control is null)
		{
			throw new InvalidOperationException("JupiterControl must be initialized before window is set");
		}

		_control.ConfigureJupiterWindow(pCoreWindow);

		//if (pCoreWindow is not null)
		//{
		//	SetCoreWindow(pCoreWindow);
		//}
		//else // CoreWindow does not exist in Win32/ Islands
		//{
		//	XamlPalSetCoreWindow();
		//}
	}
}
