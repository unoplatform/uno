...
namespace Windows.UI.Xaml.Controls
{
		...
		private void OnPropertyChanged2(DependencyPropertyChangedEventArgs args)
		{
			...
			switch (args.Property)
			{
				...
				case KnownPropertyIndex.CalendarDatePicker_Date:
				{
					...
					CValueBoxer.UnboxValue<DateTime>(args.m_pOldValue, spOldDateReference));
					CValueBoxer.UnboxValue<DateTime>(args.m_pNewValue, spNewDateReference));
				...
				case KnownPropertyIndex.CalendarDatePicker_LightDismissOverlayMode:
				{
					if (m_tpFlyout is {})
					{
						...
						var overlayMode = LightDismissOverlayMode_Off;
						overlayMode = LightDismissOverlayMode;
						m_tpFlyout.Cast<FlyoutBase>().put_LightDismissOverlayMode(overlayMode));
					}
    ...
    switch (headerPlacement)
    {
			...
			out bool* pIsEventSourceChildOfTarget)
		{
			...
			spOriginalSourceAsI.As(spOriginalSourceAsDO);
			...
			out bool pIsChildOfTarget)
		{
			...
			static DependencyObject pMostRecentSearchChildNoRef = null;
			...
			if (pChild is {})
			{
				...
				while (spCurrentDO is {} && !isFound)
				{
					...
					else if (spHeaderPresenterAsDO && spCurrentDO == spHeaderPresenterAsDO)
					{
					...
					{
						VisualTreeHelper.GetParentStatic(spCurrentDO, &spParentDO));
