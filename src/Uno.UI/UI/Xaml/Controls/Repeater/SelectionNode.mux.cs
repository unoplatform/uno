...
// MUX Reference SelectionNode.cpp, commit 4b206bce3
...
partial class SelectionNode
{
	...
	// Move this logic into the explicit unhook flow (ClearSelection / Source setter) to avoid leaks.
	...
	internal SelectionNode GetAt(int index, bool realizeChild)
	{
		...
		if (realizeChild)
		{
			...
			MUX_ASSERT(0 <= index && index <= m_childrenNodes.Count);
	...
	internal bool ToggleSelect(int index)
	{
...
#if HAS_UNO
		// Uno specific: In Uno Platform we are unhooking the handlers explicitly instead of relying on shared_ptr
		// destruction. The C++ destructor calls UnhookCollectionChangedHandler automatically when the
		// std::shared_ptr count drops to zero; in C# we must do this manually here before clearing the list.
...
#if HAS_UNO
					// Uno specific: explicitly unhook child collection-changed handler before removing
					// the node, as we don't get shared_ptr destruction semantics in C#.
	...
	{
		bool? result = null; // PartiallySelected
	...
	internal SelectionState EvaluateIsSelectedBasedOnChildrenNodes()
	{
		...
		if (realizedChildrenNodeCount != 0 || selectedCount != 0)
		{
			...
			else
			{
				...
				{
					if (GetAt(i, false /* realizeChild */) is { } child)
					{
