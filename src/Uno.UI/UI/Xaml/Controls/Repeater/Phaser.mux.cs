...
// MUX Reference Phaser.cpp, commit 4b206bce3
...
partial class Phaser
{
		...
		VirtualizationInfo virtInfo)
	{
		...
		else if (nextPhase == VirtualizationInfo.PhaseNotSpecified)
		{
			...
			MUX_ASSERT(dataTemplateComponent is null);
	...
	private void DoPhasedWorkCallback()
	{
		...
		if (m_pendingElements.Count > 0 && !BuildTreeScheduler.ShouldYield())
		{
			...
			do
			{
				...
				var pendingCount = m_pendingElements.Count;
				if (currentIndex == -1)
				{
	...
	private void RegisterForCallback()
	{
		...
		{
			MUX_ASSERT(m_pendingElements.Count > 0);
