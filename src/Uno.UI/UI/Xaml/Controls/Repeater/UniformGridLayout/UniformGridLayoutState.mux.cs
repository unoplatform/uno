...
// MUX Reference UniformGridLayoutState.cpp, commit 4b206bce3
...
partial class UniformGridLayoutState
{
		...
		uint maxItemsPerLine)
	{
		...
		if (context.ItemCount > 0)
		{
			...
			if (m_flowAlgorithm.GetElementIfRealized(0) is { } realizedElement)
			{
	...
	internal void InvalidateElementSize() => m_isEffectiveSizeValid = false;
	...
	private double CalculateExtraPixelsInLine(
		uint maxItemsPerLine,
		float availableSizeMinor,
		double itemSizeMinor,
		double minorItemSpacing)
	{
		var numItemsBasedOnSize = (uint)Math.Max(1.0, availableSizeMinor / (itemSizeMinor + minorItemSpacing));
		...
		uint maxItemsPerLine)
	{
		...
		var itemSizeMinor = orientation == Orientation.Horizontal ? m_effectiveItemWidth : m_effectiveItemHeight;
		...
		{
			var itemSizeMajor = orientation == Orientation.Horizontal ? m_effectiveItemHeight : m_effectiveItemWidth;
