...
/// <summary>
/// CoreWebView2CookieManager implementation - provides cookie management functionality
/// </summary>
public partial class CoreWebView2CookieManager
{
	...
	/// </summary>
	/// <param name="uri">The URI of the cookies to get. If empty or null, returns all cookies.</param>
	...
	public IAsyncOperation<IReadOnlyList<CoreWebView2Cookie>> GetCookiesAsync(string uri)
	{
		return AsyncOperation.FromTask(async ct =>
		{
			...
			{
				var cookies = await cookieManager.GetCookiesAsync(uri ?? string.Empty);
				return (IReadOnlyList<CoreWebView2Cookie>)cookies;
			}

			return (IReadOnlyList<CoreWebView2Cookie>)Array.Empty<CoreWebView2Cookie>();
		});
	}
	...
	public CoreWebView2Cookie CreateCookie(string name, string value, string Domain, string Path)
	{
		...
		return new CoreWebView2Cookie(name, value ?? string.Empty, Domain ?? string.Empty, Path ?? "/");
	}
	...
	public void DeleteCookies(string name, string uri)
	{
		if (_nativeWebView is INativeWebViewCookieManager cookieManager)
		{
			cookieManager.DeleteCookies(name ?? string.Empty, uri ?? string.Empty);
		}
	}
	...
	public void DeleteCookiesWithDomainAndPath(string name, string Domain, string Path)
	{
		if (_nativeWebView is INativeWebViewCookieManager cookieManager)
		{
			cookieManager.DeleteCookiesWithDomainAndPath(name ?? string.Empty, Domain ?? string.Empty, Path ?? "/");
		}
	}
