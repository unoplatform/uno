...
namespace Windows.UI.Xaml.Controls
{
	public partial class Pivot : ItemsControl
	{
		...
		protected override DependencyObject GetContainerForItemOverride()
			=> _isUWPTemplate ? new PivotItem() { CanCreateTemplateWithoutParent = true } : GetContainerForItemOverride();
			...
			if (_isUWPTemplate)
			{
				...
				foreach (var item in Items)
				{
					if (item is PivotItem pivotItem)
					{
						...
						headerItem.PointerPressed += OnItemPointerPressed;
		...
		private void OnSelectedIndexChanged(int oldValue, int newValue)
		{
			if (_isUWPTemplate)
			{
				if (newValue != -1)
				{
					var selectedHeader = _staticHeader.Children[newValue] as PivotHeaderItem;
					var selectedPivotitem = Items[newValue] as PivotItem;
