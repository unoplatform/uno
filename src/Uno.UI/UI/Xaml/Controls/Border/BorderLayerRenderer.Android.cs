...
partial class BorderLayerRenderer
{
	...
	{
		var nativeSize = _owner.LayoutSlotWithMarginsAndAlignments.Size.LogicalToPhysicalPixels();
		var newState = new BorderLayerState(
			new Size(nativeSize.Width, nativeSize.Height),
		...
		var disposable = InnerCreateLayers(_owner, newState, () => onImageSet?.Invoke());
	...
	private IDisposable InnerCreateLayers(
		BindableView view,
		BorderLayerState newState,
		Action onImageSet)
	{
		// In case the element has no size, skip everything!
		if (newState.ElementSize.Width == 0 && newState.ElementSize.Height == 0)
		{
			return Disposable.Empty;
		}

		var drawArea = new Rect(default, newState.ElementSize);

		var disposables = new CompositeDisposable();

		var physicalBorderThickness = newState.BorderThickness.LogicalToPhysicalPixels();
		var isInnerBorderSizing = newState.BackgroundSizing == BackgroundSizing.InnerBorderEdge;
		var adjustedArea = isInnerBorderSizing
			? drawArea.DeflateBy(physicalBorderThickness)
			: drawArea;

		var fullCornerRadius = newState.CornerRadius.GetRadii(drawArea.Size, newState.BorderThickness);
		...
		if (!fullCornerRadius.IsEmpty)
		{
			...
			using (var backgroundPath = new Path())
			{
				...
				{
					if (newState.Background is ImageBrush imageBrushBackground)
					{
					...
					else if (newState.Background is AcrylicBrush acrylicBrush)
					{
					...
					{
						_fillPaint ??= new();
						if (newState.Background is not null)
						{
							newState.Background.ApplyToFillPaint(drawArea, _fillPaint);
						}
						else
						{
							_fillPaint.Reset();
				...
				if (newState.BorderThickness != Thickness.Empty &&
					newState.BorderBrush is { } borderBrush &&
					borderBrush is not ImageBrush)
				{
		...
		else // No corner radius
		{
			...
			if (!drawArea.HasZeroArea())
			{
				var finalDrawArea = newState.BorderThickness != Thickness.Empty ? adjustedArea : drawArea;
				...
				else
				{
					...
					ExecuteWithNoRelayout(view, v => v.SetBackgroundDrawable(Brush.GetBackgroundDrawable(newState.Background, drawArea, _fillPaint, backgroundPath, antiAlias: false)));
				}
				disposables.Add(() => ExecuteWithNoRelayout(view, v => v.SetBackgroundDrawable(null)));
			}

			if (newState.BorderThickness != Thickness.Empty && !(newState.BorderBrush is ImageBrush))
			{
				//TODO: Handle case when BorderBrush is an ImageBrush
				// Related Issue: https://github.com/unoplatform/uno/issues/6893
				_strokePaint ??= new();
				if (newState.BorderBrush is not null)
				{
					newState.BorderBrush.ApplyToStrokePaint(drawArea, _strokePaint);
				}
				else
				{
					_strokePaint.Reset();
