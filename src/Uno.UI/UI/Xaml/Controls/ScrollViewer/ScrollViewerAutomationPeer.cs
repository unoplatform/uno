...
public partial class ScrollViewerAutomationPeer : FrameworkElementAutomationPeer, IScrollProvider
{
	...
	[NotImplemented] // Uno: [Page|Line][Left|Right|Up|Down] methods are not implemented
	public void Scroll(ScrollAmount horizontalAmount, ScrollAmount verticalAmount)
	{
		...
		switch (horizontalAmount)
		{
			case ScrollAmount.LargeDecrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).PageLeft();
				break;
			case ScrollAmount.SmallDecrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).LineLeft();
				break;
			case ScrollAmount.SmallIncrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).LineRight();
				break;
			case ScrollAmount.LargeIncrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).PageRight();
				break;
		...
		switch (verticalAmount)
		{
			case ScrollAmount.LargeDecrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).PageUp();
				break;
			case ScrollAmount.SmallDecrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).LineUp();
				break;
			case ScrollAmount.SmallIncrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).LineDown();
				break;
			case ScrollAmount.LargeIncrement:
				// Uno TODO:
				//((ScrollViewer)pOwner).PageDown();
				break;
			case ScrollAmount.NoAmount:
				break;
			default:
	...
	public void SetScrollPercent(double horizontalPercent, double verticalPercent)
	{
		...
		{
			extentWidth = ((ScrollViewer)pOwner).ExtentWidth;
			viewportWidth = ((ScrollViewer)pOwner).ViewportWidth;
			minHorizontalOffset = ((ScrollViewer)pOwner).MinHorizontalOffset;
			nettAvailable = DoubleUtil.Max(0.0, (extentWidth - viewportWidth - minHorizontalOffset));
			((ScrollViewer)pOwner).ScrollToHorizontalOffset((nettAvailable * (double)(horizontalPercent * 0.01)) + minHorizontalOffset);
		}

		if (scrollVertically)
		{
			extentHeight = ((ScrollViewer)pOwner).ExtentHeight;
			viewportHeight = ((ScrollViewer)pOwner).ViewportHeight;
			minVerticalOffset = ((ScrollViewer)pOwner).MinVerticalOffset;
			nettAvailable = DoubleUtil.Max(0.0, (extentHeight - viewportHeight - minVerticalOffset));
			((ScrollViewer)pOwner).ScrollToVerticalOffset((nettAvailable * (double)(verticalPercent * 0.01)) + minVerticalOffset);
		}
	}
	...
	public double HorizontalScrollPercent
	{
		get
		{
			...
			//if (pScrollInfo is not null)
			{
				...
				extentWidth = ((ScrollViewer)pOwner).ExtentWidth;
				viewportWidth = ((ScrollViewer)pOwner).ViewportWidth;
				horizontalOffset = ((ScrollViewer)pOwner).HorizontalOffset;
				minHorizontalOffset = ((ScrollViewer)pOwner).MinHorizontalOffset;
	...
	public double VerticalScrollPercent
	{
		get
		{
			...
			//if (pScrollInfo is not null)
			{
				...
				extentHeight = ((ScrollViewer)pOwner).ExtentHeight;
				viewportHeight = ((ScrollViewer)pOwner).ViewportHeight;
				verticalOffset = ((ScrollViewer)pOwner).VerticalOffset;
				minVerticalOffset = ((ScrollViewer)pOwner).MinVerticalOffset;
	...
	private protected override bool ChildIsAcceptable(UIElement element)
	{
		...
		if (bchildIsAcceptable)
		{
			...
			pElementHorizontalScrollBar = ((ScrollViewer)pOwner).ElementHorizontalScrollBar;
			pElementVerticalScrollBar = ((ScrollViewer)pOwner).ElementVerticalScrollBar;
