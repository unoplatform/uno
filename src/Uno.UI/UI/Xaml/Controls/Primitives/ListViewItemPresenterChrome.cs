...
// MUX Reference ListViewItemPresenter_Partial.h, ListViewItemPresenter_Partial.cpp, tag winui3/release/1.4.2
...
/// <summary>
/// Chrome helper class for ListViewItemPresenter that handles visual state management and animations.
/// Uses composition pattern to avoid changing the class hierarchy of the generated presenter.
/// </summary>
internal sealed class ListViewItemPresenterChrome : IListViewBaseItemAnimationCommandVisitor
{
	private readonly ListViewItemPresenter _presenter;
	...
	public ListViewItemPresenterChrome(ListViewItemPresenter presenter)
	{
		...
		ref bool needsArrange)
	{
		...
		var command = new ListViewBaseItemAnimationCommand_MultiSelect(
			entering,
			_presenter.CheckMode == ListViewItemPresenterCheckMode.Inline,
	...
	private void EnsureMultiSelectCheckBox()
	{
		...
		_multiSelectCheckBoxRectangle = new Border
		{
			...
			CornerRadius = _presenter.CheckBoxCornerRadius,
			HorizontalAlignment = _presenter.CheckMode == ListViewItemPresenterCheckMode.Inline
				? HorizontalAlignment.Left
				: HorizontalAlignment.Right,
			Background = _presenter.CheckBoxBrush,
			BorderBrush = _presenter.CheckBoxBorderBrush,
	...
	private void EnsureSelectionIndicator()
	{
		...
		_selectionIndicatorRectangle = new Border
		{
			...
			VerticalAlignment = VerticalAlignment.Center,
			CornerRadius = _presenter.SelectionIndicatorCornerRadius,
			Background = _presenter.SelectionIndicatorBrush,
		};
	}
	...
	private void SetBackplateBackground()
	{
		...
		{
			brush = _presenter.PressedBackground;
		}
		...
		{
			brush = _presenter.SelectedPressedBackground;
		}
