...
namespace Windows.UI.Xaml.Controls
{
	public partial class TextBox
	{
		...
		partial void OnInputScopeChangedPartial(DependencyPropertyChangedEventArgs e)
		{
			...
			if (_textBoxView != null)
			{
				_textBoxView.KeyboardType = InputScopeHelper.ConvertInputScopeToKeyboardType(((InputScope)e.NewValue).GetFirstInputScopeNameValue());

				//If SpellCheck is enabled we already set the Capitalization.
				if (!IsSpellCheckEnabled)
				{
					_textBoxView.AutocapitalizationType = InputScopeHelper.ConvertInputScopeToCapitalization(((InputScope)e.NewValue).GetFirstInputScopeNameValue());
				}
			}
		}
