...
namespace Microsoft.UI.Xaml.Markup.Reader
{
	...
	/// Mirrors the compile-time check in the XAML source generator.
	/// </summary>
	internal static class XamlNamespaceValidation
	{
		...
		public static string FormatUnsupportedClrNamespaceMessage(string prefix, string @namespace, int lineNumber, int linePosition)
		{
			var declaration = prefix.Length > 0 ? $"xmlns:{prefix}" : "xmlns";
			var clrNamespace = @namespace.Substring(ClrNamespacePrefix.Length).Split(';')[0];

			return $"The 'clr-namespace:' XAML namespace form is not supported. Replace '{declaration}=\"{@namespace}\"' with '{declaration}=\"using:{clrNamespace}\"' [Line: {lineNumber} Position: {linePosition}]";
		}
