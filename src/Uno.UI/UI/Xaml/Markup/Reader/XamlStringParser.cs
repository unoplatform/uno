		...
		private XamlMemberDefinition VisitMember(XamlXmlReader reader, XamlObjectDefinition owner)
		{
			...
			while (reader.Read())
			{
				...
				switch (reader.NodeType)
				{
					...
					default:
						throw new InvalidOperationException("Unable to process {2} node at Line {0}, position {1}".InvariantCultureFormat(Boxer.Box(reader.LineNumber), Boxer.Box(reader.LinePosition), reader.NodeType));
				}
			}
