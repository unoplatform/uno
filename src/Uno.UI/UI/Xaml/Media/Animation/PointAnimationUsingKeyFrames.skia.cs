...
namespace Microsoft.UI.Xaml.Media.Animation
{
	public partial class PointAnimationUsingKeyFrames
	{
		...
		private List<PointKeyFrame> _sortedKeyFramesCache;
		...
		private void UpdateUsingKeyFrames()
		{
			...
			var fromValue = _startingValue ?? default(Point);
			...
			if (easing != null)
			{
				...
				// For Point interpolation, apply easing to segment progress then interpolate X/Y independently.
				var easedProgress = easing.Ease(segmentProgress, 0.0, 1.0, 1.0);
				value = new Point(
					fromValue.X + (toValue.X - fromValue.X) * easedProgress,
					fromValue.Y + (toValue.Y - fromValue.Y) * easedProgress);
			}
			else
			{
				// Linear interpolation — interpolate X/Y independently.
				value = new Point(
					fromValue.X + (toValue.X - fromValue.X) * segmentProgress,
					fromValue.Y + (toValue.Y - fromValue.Y) * segmentProgress);
			}
