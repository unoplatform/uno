...
public partial class CompositionTarget
{
	...
	/// <remarks>
	/// Falls back to the primary XamlRoot so an island host — which has no Window at all — still resolves one.
	...
	internal void RaiseFrameStarting()
	{
		...
		// One try/catch per driver: a multicast invocation aborts at the first handler that throws, so a
		// single faulty driver would starve every driver behind it, on every frame, for as long as it throws.
