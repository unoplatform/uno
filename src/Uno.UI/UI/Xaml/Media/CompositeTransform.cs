...
namespace Windows.UI.Xaml.Media
{
	...
	public partial class CompositeTransform : Transform
	{
		internal override Matrix3x2 ToMatrix(Point absoluteOrigin)
		{
			...
			var transformCenterX = CenterX;
			var transformCenterY = CenterY;
			var centerX = absoluteOrigin.X + transformCenterX;
			var centerY = absoluteOrigin.Y + transformCenterY;

			var matrix = Matrix3x2.Identity;

			matrix *= ScaleTransform.GetMatrix(centerX, centerY, ScaleX, ScaleY);
			matrix *= SkewTransform.GetMatrix(transformCenterX, transformCenterY, SkewX, SkewY);
			matrix *= RotateTransform.GetMatrix(transformCenterX, transformCenterY, Rotation);
