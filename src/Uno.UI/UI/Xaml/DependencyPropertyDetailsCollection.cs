	...
	partial class DependencyPropertyDetailsCollection : IDisposable
	{
		...
		private DependencyPropertyDetails? TryGetPropertyDetails(DependencyProperty property, bool forceCreate)
		{
			...
			if (isInRange)
			{
				...
				{
					propertyEntry = new DependencyPropertyDetails(property, _ownerType, isTemplatedParentOrDataContext: property == _dataContextProperty || property == _templatedParentProperty);
