...
internal readonly partial struct UnicodeText : IParsedText
{
	...
	private static readonly Lazy<ISpellCheckingService?> _spellCheckingService = new(() =>
	{
		...
		{
			typeof(UnicodeText).LogWarn()?.Warn($"No implementation of {nameof(ISpellCheckingService)} was found. Spell checking will be disabled. To enable spell checking, add the '{{nameof(SpellChecking)}}' UnoFeature.");
			return null;
		}
	});
