...
internal readonly partial struct UnicodeTextNew : IParsedText
{
	...
	{
		if (ApiExtensibility.CreateInstance<ISpellCheckingService>(typeof(UnicodeTextOld), out var service))
		{
			return service;
		}
		else
		{
			typeof(UnicodeTextOld).LogError()?.Error($"No implementation of {nameof(ISpellCheckingService)} was found. Spell checking will be disabled.");
			return null;
		}
	});
