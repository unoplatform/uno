...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Automation
{
	...
	public class Given_LandmarkPromotion
	{
		...
		public void When_Peer_Requested_Before_LocalizedLandmarkType_Set_Then_Promoted()
		{
			var grid = new Grid();

			Assert.IsNull(FrameworkElementAutomationPeer.CreatePeerForElement(grid));

			AutomationProperties.SetLocalizedLandmarkType(grid, "Primary navigation");

			Assert.IsNotNull(
				FrameworkElementAutomationPeer.CreatePeerForElement(grid),
				"A null peer result must not prevent later landmark promotion");
		}
