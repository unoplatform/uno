	...
	public async Task PropagatePropertiesDown()
	{
		...
		try
		{
			...
			RunOnUIThread.Execute(() =>
			{
				...
				var template = tip.Template;
				var style = tip.Style;

				tip.RegisterPropertyChangedCallback(TeachingTip.TemplateProperty, (_, _) => { });
				tip.RegisterPropertyChangedCallback(TeachingTip.StyleProperty, (_, _) => { });
