...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Media_Imaging
{
	...
	public class Given_BitmapSource
	{
		...
		public async Task When_LocalResource_ScaleQualifier()
		{
			...
			Assert.Contains(resolved.PathAndQuery, "scale-400", $"Resolved asset path did not contain the expected .scale-400 qualifier: {resolved}");
		}
#endif
		...
		public async Task When_BundleImages_Loaded_StreamsAreDisposed()
		{
			...
			{
				Assert.IsGreaterThan(bitmapImage.PixelWidth, 0, "Image should have valid pixel dimensions after loading");
			}
