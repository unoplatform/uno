...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Automation
{
	...
	public class Given_AccessibleAria
	{
		...
		public async Task When_AutomationId_On_Wasm_Then_XamlAutomationId_Attribute_Is_Set()
		{
			...
			AutomationProperties.SetAutomationId(button, "updated-button-id");
			await UITestHelper.WaitFor(() => GetSemanticAttribute(button, "xamlautomationid") == "updated-button-id", timeoutMS: 3000,
				message: "A direct AutomationId change did not update xamlautomationid.");

			AutomationProperties.SetAutomationId(button, string.Empty);
			await UITestHelper.WaitFor(() => !SemanticElementHasAttribute(button, "xamlautomationid"), timeoutMS: 3000,
				message: "Clearing AutomationId left stale xamlautomationid metadata.");
		}
		...
		public async Task When_LiveRegion_Then_AriaLive_Set_But_No_AriaAtomic()
		{
			...
			await UITestHelper.WaitFor(() => GetSemanticAttribute(status, "aria-live") == "assertive", timeoutMS: 3000,
				message: "A direct LiveSetting change did not update aria-live.");

			AutomationProperties.SetLiveSetting(status, AutomationLiveSetting.Off);
			await UITestHelper.WaitFor(() => !SemanticElementHasAttribute(status, "aria-live"), timeoutMS: 3000,
				message: "Clearing LiveSetting left stale aria-live.");
		}

		[TestMethod]
		[RunsOnUIThread]
		[PlatformCondition(ConditionMode.Include, RuntimeTestPlatforms.SkiaWasm)]
		public async Task When_Shortcut_Properties_Change_Then_Dom_Metadata_Live_Syncs()
		{
			var button = new Button { Content = "Save" };
			...
			EnableAccessibilityThroughDom();
			await UITestHelper.WaitFor(() => SemanticElementExists(button), timeoutMS: 5000,
				message: "Timed out waiting for the shortcut-bearing Button semantic node.");

			AutomationProperties.SetAcceleratorKey(button, "Ctrl+Shift+S");
			AutomationProperties.SetAccessKey(button, "S");
			await UITestHelper.WaitFor(
				() => GetSemanticAttribute(button, "aria-keyshortcuts") == "Ctrl+Shift+S" && GetSemanticAttribute(button, "accesskey") == "S",
				timeoutMS: 3000,
				message: "Direct shortcut-property changes did not reach the semantic DOM.");

			AutomationProperties.SetAcceleratorKey(button, string.Empty);
			AutomationProperties.SetAccessKey(button, string.Empty);
			await UITestHelper.WaitFor(
				() => !SemanticElementHasAttribute(button, "aria-keyshortcuts") && !SemanticElementHasAttribute(button, "accesskey"),
				timeoutMS: 3000,
				message: "Clearing shortcut properties left stale DOM metadata.");
		}
		...
		public async Task When_Named_Control_With_Authored_LocalizedType_Then_RoleDescription_Emitted()
		{
			...
			AutomationProperties.SetLocalizedControlType(button, "transport control");
			await UITestHelper.WaitFor(() => GetSemanticAttribute(button, "aria-roledescription") == "transport control", timeoutMS: 3000,
				message: "A direct LocalizedControlType change did not update aria-roledescription.");

			AutomationProperties.SetLocalizedControlType(button, string.Empty);
			await UITestHelper.WaitFor(() => !SemanticElementHasAttribute(button, "aria-roledescription"), timeoutMS: 3000,
				message: "Clearing LocalizedControlType left stale aria-roledescription.");
		}
