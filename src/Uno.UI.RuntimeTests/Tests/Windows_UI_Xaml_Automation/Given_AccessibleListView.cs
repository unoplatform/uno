...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Automation
{
	...
	public class Given_AccessibleListView
	{
		...
		[RunsOnUIThread]
		[PlatformCondition(ConditionMode.Include, RuntimeTestPlatforms.SkiaWasm)]
		public async Task When_Virtualized_Item_Is_Removed_Then_Relationship_Authority_Is_Removed()
		{
			var source = new Button { Content = "Source" };
			var items = new ObservableCollection<string> { "Old item" };
			var listView = new ListView
			{
				ItemsSource = items,
				Width = 200,
			...
			await UITestHelper.WaitFor(
				...
				timeoutMS: 3000,
				message: "Removing a virtualized item retained its semantic node or inverse-flow authority.");

			items.Add("Replacement item");
			await UITestHelper.WaitFor(
				() => listView.ContainerFromIndex(0) is UIElement replacement && SemanticElementExists(replacement),
				timeoutMS: 3000,
				message: "A replacement item at the same index did not receive a clean semantic node.");
		}

		[TestMethod]
		[RunsOnUIThread]
		[PlatformCondition(ConditionMode.Include, RuntimeTestPlatforms.SkiaWasm)]
		public async Task When_SelectionMode_Changes_Then_AriaMultiselectable_LiveSyncs()
		{
			var listView = new ListView
			{
			...
			listView.GetOrCreateAutomationPeer();
			EnableAccessibilityThroughDom();
			await UITestHelper.WaitFor(() => SemanticElementExists(listView), timeoutMS: 5000,
				message: "Timed out waiting for the list semantic node.");

			Assert.AreEqual("true", GetSemanticAttribute(listView, "aria-multiselectable"));

			listView.SelectionMode = ListViewSelectionMode.Single;
			await UITestHelper.WaitFor(
				() => !SemanticElementHasAttribute(listView, "aria-multiselectable"),
				timeoutMS: 3000,
				message: "Changing SelectionMode to Single left stale aria-multiselectable.");
		}
		...
		public async Task When_ListItem_Position_Becomes_Unknown_Then_Set_Position_Attributes_Are_Removed()
		{
			...
			item.SizeOfSet = 0;
			peer.RaisePropertyChangedEvent(AutomationElementIdentifiers.PositionInSetProperty, 2, 0);
			peer.RaisePropertyChangedEvent(AutomationElementIdentifiers.SizeOfSetProperty, 5, 0);
			await UITestHelper.WaitFor(
