...
public class Given_CalendarDatePicker
{
	...
	public async Task TestCalendarPanelSize()
	{
		...
		Assert.IsGreaterThan(calendarView.ActualHeight, 300);
	...
	public async Task When_Theme_Changes()
	{
		...
		try
		{
			...
			foreach (var item in items)
			{
				...
				Assert.IsLessThan(foreground.Luminance, 0.5);
				...
				{
					Assert.IsGreaterThan(background.Luminance, 0.5);
				}
			}
			...
			{
				var foreground = ((SolidColorBrush)item.Foreground).Color;
				Assert.IsGreaterThan(foreground.Luminance, 0.5);
				...
				if (background.R == background.G && background.G == background.B && background.A != 0)
				{
					Assert.IsLessThan(background.Luminance, 0.5);
				}
			}
		}
		finally
