...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls
{
	...
	public class Given_TextBlock
	{
		...
		public void Check_ActualWidth_After_Measure()
		{
			...
			SUT.Measure(size);
			Assert.IsGreaterThan(SUT.DesiredSize.Width, 0);
			Assert.IsGreaterThan(SUT.DesiredSize.Height, 0);
			...
			Assert.IsGreaterThan(SUT.ActualWidth, 0);
			Assert.IsGreaterThan(SUT.ActualHeight, 0);
		}
		...
		public async Task When_Empty_TextBlocks_Stacked()
		{
			...
			for (int i = 1; i < container.Children.Count; i++)
			{
				...
				Assert.IsLessThan(previousOrigin.Y, textBlockOrigin.Y);
