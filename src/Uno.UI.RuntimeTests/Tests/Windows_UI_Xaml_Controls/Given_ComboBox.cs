...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls
{
	...
	public class Given_ComboBox
	{
		...
		public async Task When_ComboPopup_Rearrange_ScrollShouldNotReset()
		{
			...
			await UITestHelper.WaitForIdle();
			Assert.IsLessThan(Math.Abs(destination - sv.VerticalOffset), 1.0, $"Expect sv.VerticalOffset to be near {destination:0.##}, got: {sv.VerticalOffset:0.##}");

			// force an arrange
			var cbi = sv.FindFirstChild<ComboBoxItem>();
			cbi.InvalidateArrange();
			await UITestHelper.WaitForIdle();
			Assert.IsLessThan(Math.Abs(destination - sv.VerticalOffset), 1.0, $"Expect sv.VerticalOffset to be still near {destination:0.##}, got: {sv.VerticalOffset:0.##}");
		}
#endif
