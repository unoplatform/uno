...
public class Given_Panel
{
	...
	public async Task When_Overriding_Measure_Arrange()
	{
		...
		Assert.IsLessThan(Math.Abs(parent.ActualSize.X - lv.ActualSize.X), 1);
		Assert.IsLessThan(Math.Abs(parent.ActualSize.Y - lv.ActualSize.Y), 1);
		...
		for (var i = 0; i < 4; i++)
		{
			...
			{
				Assert.IsLessThan(Math.Abs(child.DesiredSize.Height - child.ActualSize.Y), 1);
			}
			else
			{
				Assert.IsLessThan(Math.Abs(child.DesiredSize.Width - child.ActualSize.X), 1);
			}
		}
	}
}
