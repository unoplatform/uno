...
public class Given_MrtCoreAssemblyIdentity
{
	...
	// The Uno.WinRT package ships this assembly as "Uno" - the package id is not the assembly name.
	...
	public void When_MrtCoreType_Then_Declared_In_The_Uno_Assembly(Type type)
		=> Assert.AreEqual(
	...
	[TestMethod]
	public void When_Legacy_WinRT_ResourceLoader_Then_Also_In_The_Uno_Assembly()
		=> Assert.AreEqual(
			ExpectedAssemblyName,
			typeof(global::Windows.ApplicationModel.Resources.ResourceLoader).Assembly.GetName().Name,
			"The pre-MRT loader has always lived in the Uno assembly; the migration note says so explicitly.");
