...
public class Given_ContainerVisual
{
	...
	public async Task When_Ahead_Of_Time_Frame_Becomes_Stale_Rerecords_Immediately()
	{
		...
		try
		{
			...
			var compositionTarget = (CompositionTarget)border.Visual.CompositionTarget!;
			using var surface = SKSurface.Create(new SKImageInfo(1024, 768));
			var dispatcherCallbacks = new Queue<Action>();
			compositionTarget.RenderCallbackSchedulerForTesting = dispatcherCallbacks.Enqueue;
			try
			{
				// Drain a possible initial ahead-of-time frame and its single deferred request.
				for (var i = 0; i < 2; i++)
				{
					compositionTarget.OnNativePlatformFrameRequested(surface.Canvas, _ => surface.Canvas);
					DrainDispatcherCallbacks();
				}

				border.Background = new SolidColorBrush(Colors.Green);
				compositionTarget.OnRenderFrameOpportunity();
				...
				compositionTarget.OnNativePlatformFrameRequested(surface.Canvas, _ => surface.Canvas);
				DrainDispatcherCallbacks();
				AssertPresentedColor(SKColors.Green);

				surface.Canvas.Clear(SKColors.Transparent);
				compositionTarget.OnNativePlatformFrameRequested(surface.Canvas, _ => surface.Canvas);
				DrainDispatcherCallbacks();
				AssertPresentedColor(SKColors.Blue);
				...
				((FrameworkElement)TestServices.WindowHelper.XamlRoot.VisualTree.RootElement!).UpdateLayout();
				compositionTarget.OnRenderFrameOpportunity();
				surface.Canvas.Clear(SKColors.Transparent);
				compositionTarget.OnNativePlatformFrameRequested(surface.Canvas, _ => surface.Canvas);
				DrainDispatcherCallbacks();
				...
				void AssertPresentedColor(SKColor expected)
				{
					var bounds = border.GetAbsoluteBoundsRect();
					var scale = border.XamlRoot?.RasterizationScale ?? 1;
					using var image = surface.Snapshot();
					using var bitmap = SKBitmap.FromImage(image);
					var actual = bitmap.GetPixel(
						(int)Math.Round((bounds.X + bounds.Width / 2) * scale),
						(int)Math.Round((bounds.Y + bounds.Height / 2) * scale));
					Assert.AreEqual(expected, actual);
				}

				void DrainDispatcherCallbacks()
				{
					while (dispatcherCallbacks.TryDequeue(out var callback))
					{
						callback();
					}
				}
			...
			finally
			{
				...
				{
					compositionTarget.RenderCallbackSchedulerForTesting = null;
				}
			}
		}
		finally
