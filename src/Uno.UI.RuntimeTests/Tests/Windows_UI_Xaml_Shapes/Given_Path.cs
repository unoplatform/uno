...
namespace Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Shapes
{
	...
	public class Given_Path
	{
		...
		[PlatformCondition(ConditionMode.Exclude, RuntimeTestPlatforms.NativeWinUI)]
		public async Task When_Path_In_Nested_Canvas_With_MatrixTransform_Loads_Without_Exception()
		{
			// Issue: a Path inside nested Canvases with MatrixTransform and TranslateTransform
			// doesn't render correctly on WASM (visual mismatch with UWP).
			// This only guards the layout pass: loading must not throw and the Canvas must keep its
			// explicit bounds. Pixel-level validation of the transformed geometry is not covered here.
			...
			try
			{
				...
				Assert.IsNull(caught,
					$"Expected the path in nested Canvases with MatrixTransform to load without exception, but got: {caught}");

				Assert.IsNotNull(canvas, "Canvas layer1 should be found in visual tree.");
				Assert.IsTrue(canvas.ActualWidth > 0 && canvas.ActualHeight > 0,
					$"Expected Canvas to have non-zero size after rendering, got {canvas.ActualWidth}x{canvas.ActualHeight}.");
			}
			finally
			{
