...
#else
			Assert.IsLessThanOrEqualTo(Math.Abs(11 - SUT.DesiredSize.Width), 1, $"Actual size: {SUT.DesiredSize}");
			Assert.IsLessThanOrEqualTo(Math.Abs(50 - SUT.DesiredSize.Height), 1, $"Actual size: {SUT.DesiredSize}");
#endif
		}
