	...
	public class Given_DispatcherQueueTimer
	{
		...
		public async Task When_Sleep_In_Tick()
		{
			...
			try
			{
				...
				Assert.IsGreaterThanOrEqualTo(stopwatch.ElapsedMilliseconds, 180);
			}
			finally
			{
				dispatcherTimer.Stop();
			...
			try
			{
				...
				dispatcherTimer.Tick += (s, e) =>
				{
					...
					if (repeats == 1)
					{
						...
						Assert.IsLessThanOrEqualTo(stopwatch.ElapsedMilliseconds, 400);
						stopwatch.Restart();
					}
					else if (repeats == 2)
					{
						// The second tick should be scheduled after 600ms
						Assert.IsGreaterThanOrEqualTo(stopwatch.ElapsedMilliseconds, 400);
						stopwatch.Stop();
						dispatcherTimer.Stop();
					}
				};
		...
		public async Task When_Exception_In_Tick()
		{
			...
			try
			{
				...
				Assert.IsGreaterThan(tickCounter, 1);
			}
			finally
			{
				dispatcherTimer.Stop();
