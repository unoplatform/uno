	...
	public class Given_DoubleAnimation
	{
		...
		public async Task When_RepeatForever_ShouldLoop() // Flaky - #9080
		{
			async Task Do()
			{
				...
				Assert.IsGreaterThanOrEqualTo(incrementSizes.Count(x => x >= 3), 8, $"Expected at least 10sets (-2 error margin: might miss first and/or last) of continuous increments in size of 4 (+-1 error margin: sliding slot).\n" + context);
		...
		public async Task When_StartingFrom_AnimatedValue() // value from completed(filling) animation
		{
			...
			Assert.IsGreaterThan(y, 0, $"Expecting Translate.Y to be still positive: {y}");
		}
		...
		public async Task When_StartingFrom_AnimatingValue() // value from mid animation
		{
			...
			if (value is double y)
			{
				...
				Assert.IsGreaterThan(y, 50, $"Expecting Translate.Y to be still positive: {y}");
			}
			else
			{
