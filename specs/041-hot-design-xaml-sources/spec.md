...
- **FR-008 — Path canonicalization**: Hot Reload hosts MUST ensure that every physical file is registered in the Roslyn `Workspace` under a single canonical path. Paths arriving at workspace-mutation boundaries (MSBuild item expansion, file-system watcher events, RPC-driven file notifications, agent-tool writes, debugger-hosted updates) often differ in separator style (`\` vs `/`), casing on case-insensitive filesystems, and presence of `.`/`..` segments or redundant separators. Roslyn's `Workspace` compares `Document.FilePath` by string equality and does NOT deduplicate across these variants — so `C:\App\MainPage.xaml` and `C:/App/MainPage.xaml` coexist as two `AdditionalDocument`s pointing at the same on-disk file. When that happens, `XamlCodeGenerator` enumerates both, emits duplicate `GetSources_N()` methods into `EmbeddedXamlSourcesProvider`, and the resulting C# compilation fails with duplicate type definitions. Compile errors then accumulate until Roslyn's `WatchHotReloadService.EmitSolutionUpdateAsync` refuses to emit any IL delta, and the inner app silently stops receiving Hot Reload updates. Hosts SHOULD canonicalize inbound paths (e.g. via `Path.GetFullPath`) at every workspace entry point, and SHOULD use a separator-agnostic path comparer (case-aware per OS) when matching incoming events against existing `Documents` / `AdditionalDocuments` to reuse existing `DocumentId`s rather than creating duplicates.
- **FR-009 — Method-group delegates**: getters in `_XamlSources` MUST remain method-group references to private static methods (no lambdas, no closures), preserving HR compatibility. The HR runtime relies on this to retarget delegate invocations to the newly-emitted method bodies.
- **FR-010 — Release configuration**: the provider is NOT emitted when `UnoGenerateXamlSourcesProvider` is false/unset. Unchanged from historical behaviour.
- **FR-011 — Clean operation**: the provider MUST NOT write to `Console.Out` or `Console.Error` under any code path. Any diagnostics MUST flow through the Uno logging channel.
- **FR-012 — Snapshot test parity**: the generator output snapshots under `src/SourceGenerators/Uno.UI.SourceGenerators.Tests/XamlCodeGeneratorTests/Out/Given_HotReloadEnabledInBuild/**` and `Out/Given_GenerateEmbeddedXamlSources/**` MUST match the generator's current emission. Any change to the emission format requires updating the snapshots in the same commit.
...
- **SC-006 — No duplicate workspace documents**: across an uninterrupted Hot Reload session with repeated edits, the `Workspace`'s `AdditionalDocuments` contain exactly one entry per tracked `.xaml` file (modulo per-TFM project duplication). Host implementations can verify this by enumerating `project.AdditionalDocuments` after a sequence of edits and asserting no two entries resolve to the same canonical path.
...
- **Duplicate `AdditionalDocument` via inconsistent path spelling**: separately from the generated-provider contract, Hot Reload hosts have been observed to register the same physical file twice under different separator styles — `C:\App\MainPage.xaml` and `C:/App/MainPage.xaml` — silently coexisting in the `Workspace`. Generators then enumerate both and emit conflicting output. FR-008 codifies the host-side responsibility to canonicalize paths and match existing documents by canonical form.
