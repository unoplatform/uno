...
```
ParseTargetsForAddIns(packagesJsonPath, projectAssetsPath, nugetCachePaths[]):
  1. Build merged package list from two sources:
     a. Parse packages.json -> (packageName, version) from Uno SDK
     b. Parse project.assets.json -> ALL (packageName, version) from NuGet restore
     c. Merge: union (deduplicate by packageName; if both sources list the
        same package with different versions, prefer project.assets.json
        because it reflects the actual restored version after NuGet resolution)
     If project.assets.json is missing: use packages.json only + warn
  2. For each (packageName, version) in merged list:
     ...
     c. For each .targets file:
        i.  Try parse as XML (XDocument.Parse)
            → On failure (malformed XML, encoding error, BOM issue):
               log warning with file path and exception message,
               skip this file, continue to next .targets file.
        ...
        iii. Resolve <PropertyGroup> elements:
            - For each property with a simple value containing $(...) references,
              substitute from the property dictionary and store the result
            - Property substitution depth limit: **max 5 levels**.
              If a property still contains $(...) after 5 substitution passes,
              log warning and use the partially-resolved value as-is.
              (Known patterns use 1-2 levels; 5 is a safety margin.)
        ...
        v.  Substitute property references in Include value (same depth limit)
        vi. Normalize path (resolve ..), verify DLL exists on disk
        vii. If valid, add to result set
  3. Return { resolvedPaths[], warnings[] }
```

**Why two sources**: `packages.json` only covers Uno SDK-bundled packages. Third-party add-ins referenced directly by the project appear only in `project.assets.json` (`{projectDir}/obj/project.assets.json`), which is generated by `dotnet restore` and lists ALL packages. See main spec section 7 for details.

### `project.assets.json` Parsing (Minimal Schema)

The parser only needs the `libraries` section. The full NuGet lock file format is much larger, but only the package name and version are required for add-in discovery:
...
1. Parse as JSON, navigate to `$.libraries`
2. For each key in `libraries`: split on `/` → `(packageName, version)`
3. Filter: only entries where `type == "package"` (skip `type == "project"`)
4. The `path` field is the relative path in the NuGet cache (lowercase). Use it for directory lookup if available; otherwise construct from `{packageName.ToLower()}/{version}`.

**Resilience**: If `project.assets.json` is missing (no restore run), malformed, or uses an unknown `version` number, the parser MUST fall back to `packages.json` only and log a warning. Do NOT fail the entire discovery.

**Why this works for forward compatibility**: Any package — Uno SDK or third-party — that follows the convention (`buildTransitive/*.targets` + `UnoRemoteControlAddIns` item + `tools/devserver/` layout) is discovered automatically. No code changes, no hardcoded lists.
...
The `.targets` parser MUST normalize all resolved paths for the current OS:

- **`$(MSBuildThisFileDirectory)`** includes a trailing separator. On Windows this is `\`, on Linux/macOS `/`. The `.targets` XML may contain either. Use `Path.Combine()` for concatenation (never string concatenation with hardcoded separators).
- **Relative path segments** (`../tools/devserver/`) must be resolved via `Path.GetFullPath()` to normalize `..` and mixed separators.
- **NuGet cache lookup** must be case-insensitive on all platforms. NuGet stores package directories in lowercase (`uno.ui.app.mcp`), but `packages.json` and `project.assets.json` may reference the package with mixed case (`Uno.UI.App.Mcp`). Use `StringComparison.OrdinalIgnoreCase` for directory matching, or enumerate and match case-insensitively on Linux.
- **`exists()` conditions** in `.targets` files use the path as-is. The parser must normalize the path before checking `File.Exists()` / `Directory.Exists()`.

See [Appendix G](spec-appendix-g-compatibility-matrix.md) section 3 for the full OS compatibility matrix and [Appendix C](spec-appendix-c-testing.md) tests #24-26 for cross-platform test scenarios.
...
```
CheckDirectoriesForUnresolvedAddIns(mergedPackageList, nugetCachePaths[], alreadyResolved[]):
  For each (packageName, version) in mergedPackageList:
...
- `.targets` file MUST be in `buildTransitive/` for new add-ins
...
> **Legacy `build/` fallback**: The discovery algorithm (section 2, step 2b) falls back to `build/*.targets` when `buildTransitive/` contains no `.targets` files. This is a **legacy compatibility path** for pre-existing packages that haven't migrated to `buildTransitive/`. Packages discovered via `build/` will emit a deprecation warning in diagnostics. New add-ins MUST NOT use `build/` — it does not support transitive dependency resolution and may cause issues in complex project graphs.
...
```
For each package in mergedPackageList (packages.json + project.assets.json):
