...
4. `project.assets.json` (`{projectDir}/obj/project.assets.json`) is generated by `dotnet restore` and contains all packages (SDK + third-party). Format is stable across .NET SDK versions.
...
- [ ] Third-party add-in (not in Uno SDK `packages.json`) → discovered via `project.assets.json`
- [ ] Missing `project.assets.json` (no restore) → SDK add-ins still discovered via `packages.json`, warning emitted
