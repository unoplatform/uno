...
**`project.assets.json` location**: `{projectDir}/obj/project.assets.json`. The CLI resolves the project directory from the solution path (first `.csproj` in `.sln`, or `--project` flag if added). If the file is missing (restore not run), fall through to `packages.json`-only mode with a diagnostic warning.

**Performance**: Scanning all packages sounds expensive, but the filter is fast — most packages have no `buildTransitive/*.targets` files at all. Only packages with `.targets` containing `UnoRemoteControlAddIns` are actual add-ins. In practice, a typical Uno project has ~100-200 packages; checking for `.targets` file existence is ~1ms total.
...
| **Fire-and-forget connection** | `McpClientProxy.cs:35` | `OnServerStarted` wraps `ConnectOrDieAsync` in `Task.Run`. The `catch` block logs the error, but **does not complete the TCS on failure** — anything awaiting `UpstreamClient` hangs forever. | Complete TCS with error state in the `catch` block, or add retry logic with bounded attempts. |
...
function Test-DiscoJsonOutput {
    ...
    foreach ($addIn in $json.addIns) {
        if (-not $addIn.path -or -not (Test-Path $addIn.path)) {
            throw "disco --json add-in path does not exist: $($addIn.path)"
        }
        Write-Log " Add-in: $($addIn.path) (method: $($addIn.discoveryMethod))"
    }
