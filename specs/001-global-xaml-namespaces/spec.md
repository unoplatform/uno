...
**Created**: 2026-03-04
**Status**: Draft
**Input**: User description: "Add support for global/implicit XAML namespaces. Investigate the approach in MAUI and have this work in all Uno Platform targets"
...
1. **Given** a new Uno Platform project with global namespaces enabled, **When** a developer creates a XAML page with only `x:Class` and no explicit `xmlns` declarations, **Then** the XAML compiles successfully and the page renders correctly on all targets (WebAssembly, Skia Desktop, Android, iOS).
...
- **FR-007**: The feature MUST work across all Uno Platform targets: WebAssembly, Skia (Windows, macOS, Linux), Android, and iOS.
...
- **SC-004**: XAML Hot Reload continues to work correctly for files using implicit namespaces.
...
- The Uno Platform XAML source generator is the primary compilation path for XAML on all Uno Platform targets, and modifications to it will cover those targets.
