...
- [X] T008 Add `InjectImplicitXmlns` method to `src/SourceGenerators/Uno.UI.SourceGenerators/XamlGenerator/XamlFileParser.cs` — when `enableImplicitNamespaces` is true, inject missing `xmlns` declarations (default, `xmlns:x`, and `XmlnsPrefix`-registered prefixes) into the root element's opening tag before parsing; the `XmlReader` pipeline remains unchanged
