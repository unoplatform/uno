...
| W4 | Native WASM DOM rendering | ✅ done: 114 `.wasm.cs` deleted + validated (WebAssembly.Browser host lib net9.0 clean); `Uno.UI.Runtime.WebAssembly` project deleted; the `.ts`/`.css` DOM layer removed (`WindowManager.ts`, `HtmlDom.ts`, `MonoSupport.ts`, `Interop/**`, `Windows/**`, `tsBindings/**`, `WasmCSS/Uno.UI.css`). The kept set is exactly what the live tsconfigs reference: `ts/types/**` (browser head + `Uno.WinRT.Wasm`) and `ts/Windows/UI/Xaml/Controls/WebView.ts` (browser head). The earlier "keep bootstrap/interop `Runtime/Xaml/input/focus`" note was stale — no tsconfig or csproj references those, and the browser head ships its own `Runtime/**`. |
