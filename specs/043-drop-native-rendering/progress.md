...
| W4 | Native WASM DOM rendering | ✅ done: 114 `.wasm.cs` deleted + validated (WebAssembly.Browser host lib net9.0 clean); `Uno.UI.Runtime.WebAssembly` project deleted; the `.ts`/`.css` DOM layer removed (`WindowManager.ts`, `HtmlDom.ts`, `MonoSupport.ts`, `Interop/**`, `Windows/**`, `tsBindings/**`, `WasmCSS/Uno.UI.css`). The two survivors then moved to their consumers and `src/Uno.UI/ts` is gone: `ts/types/**` (ambient `.d.ts` shared by the browser head and `Uno.WinRT.Wasm`) now lives in `src/Uno.WinRT/ts/types`, and `WebView.ts` in the browser head's own `ts/Runtime/WebView.ts`, compiled via its `ts/**` glob. The earlier "keep bootstrap/interop `Runtime/Xaml/input/focus`" note was stale — no tsconfig or csproj references those, and the browser head ships its own `Runtime/**`. |
