...
| W5 | Core shared abstractions (`#if` cleanup) | 🟡 in progress: removed `IShadowChildrenProvider` + 6 orphaned fully-native files + cleaned ~62 shared files of dead native `#if` (batches 1-2) via build-gated workflow. REMAINING: ~178 shared `.cs` files carry now-dead native `#if` blocks (batch 3 running). **Validation gate = DIRECT `dotnet build Uno.UI.Skia.csproj`+Reference+Tests** (the `.slnf` bails early on the missing `wasm-tools-net9` workload and does NOT recompile). Host-linked files + FeatureConfiguration excluded (native branches live in host builds / W8). Skia-neutral (branches already compile-excluded) → cosmetic/contributor-readability (#12). **Not safely bulk-automatable** (shared files mix live + dead branches; `!__SKIA__`/`#else` branches are used by Reference — must preserve; per-file `#if`/`#elif`/`#else` collapse risk). Best done as a reviewed pass with Skia+Reference+Tests validation. Also: remove `NativeOwner` from Reference/unittests (rework `ElementCompositionPreview` `#else`). |
