...
- ✅ **SH-02** — Shared `NotifyPropertyChangedEventCore` + macOS `NotifyAutomationEvent` resolve the
  ...
  Win32. macOS-only in effect (verify on macOS with VoiceOver / Accessibility Inspector).
- ✅ **SH-03** — `RaiseStructureChangedEvent` routes its change type and child (Skia) into the
  per-backend structure path, so custom peers overriding GetChildrenCore can signal changes. Win32
  preserves the exact UIA structure-change type and removed-child runtime id; macOS posts
  children-changed. Test: `Given_StructureChangedEvent`.
...
Phase 5 (macOS — code-review + compile validated; runtime steps to be verified on macOS with VoiceOver / Accessibility Inspector):
- ✅ **MAC-06** — `AutomationProperties.AutomationId` exposed as NSAccessibility
  `accessibilityIdentifier` (unblocks XCUITest/Appium) across P/Invoke + `UNOAccessibility.h/.m` +
  `ApplyAttributes`, including clearing a previously assigned identifier when AutomationId is removed.
...
- ✅ **MAC-03** — `OnChildAdded`/`OnChildRemoved` coalesce a single window-level
  ...
  (reuses the existing native call); macOS-only effect (verify on macOS with VoiceOver).
