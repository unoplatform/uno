...
4. **Check DependencyProperty contracts (`.claude/rules/dependency-properties.md`).** A DP's name string, default value, and `FrameworkPropertyMetadataOptions` (`AffectsMeasure`/`AffectsArrange`/`AffectsRender`/`Inherits`) are part of its behavioral contract. Changing a default value, dropping `AffectsMeasure`, or swapping the changed/coerce callback order changes runtime behavior with no compiler warning. For `[GeneratedDependencyProperty]`, the contract lives in the attribute arguments on the partial property (or static partial `GetX` method), and callbacks are wired by name: adding, renaming or removing an `OnXChanged`/`CoerceX`/`GetXDefaultValue` method changes behavior even without `ChangedCallback = true`. The generated `XProperty` takes the accessibility of the CLR property unless an explicit `static partial DependencyProperty XProperty { get; }` declaration sets it, so changing the CLR property's accessibility, or adding/removing that declaration, can change the identifier's public surface too.
...
- **`[GeneratedDependencyProperty]` & manual `Register` (`.claude/rules/dependency-properties.md`):** Generated `…Property` identifiers (static get-only properties) and the implemented partial CLR accessors are expected public surface — don't flag them as gratuitous. Do flag default-value/metadata/callback-order changes as behavioral contract changes.
