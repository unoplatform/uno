...
1. **Map the change's blast radius.** Which assemblies, layers, and boundaries does it touch? `Uno.UI` (UI framework), `Uno.WinRT` / `Uno.Foundation` (non-UI WinRT APIs), `Uno.UI.Runtime.Skia.*`, `SourceGenerators`? Does a change in a higher layer reach down only into layers below it?
...
This is the Uno Platform framework — a single C#/XAML codebase implementing the WinUI 3 API, rendered with Skia on every target. Apply these lenses:

- **Skia-only UI (AGENTS.md → "Development scope", `.claude/rules/platform-targeting.md`):** `Uno.UI` compiles once, for Skia, and that assembly runs on every platform; the native Android View, UIKit and WASM DOM renderers were removed in 7.0. Flag changes that reintroduce a native UI path: native-view inheritance, DOM rendering, or `.Android.cs`/`.UIKit.cs`/`.wasm.cs` partials that expect the `Uno.UI` project to compile them (it excludes those suffixes by default). Platform runtime projects may still link suffixed files from under `src/Uno.UI` — e.g. `Uno.UI.Runtime.Skia.WebAssembly.Browser` compiles the `NativeWebView` `.wasm.cs` partials — so judge a file by its owning MSBuild project (`Compile Include`/`Link`), not its physical path. This applies to the UI layer only — `Uno.WinRT`/`Uno.Foundation` non-UI WinRT APIs keep per-platform implementations and are still actively enhanced.
- **Platform-specialization mechanism (`.claude/rules/platform-targeting.md`):** Is the *narrowest fitting* mechanism chosen? File suffixes (`.skia.cs`/`.Android.cs`/`.UIKit.cs`/`.wasm.cs`/`.reference.cs`/`.crossruntime.cs`) apply only in projects with per-platform variants. The Skia-only UI layer uses `OperatingSystem.IsX()` runtime checks or `ApiExtensibility`. Flag `OperatingSystem.IsAndroid()` used for compile-time exclusion, `#if __ANDROID__` in `Uno.UI` or a `.skia.cs` file, or logic dumped into a cross-platform file that belongs in a suffixed partial of a per-platform project.
- **Layering & DependencyObject:** `DependencyObject` is a plain class on every target (WinUI parity) — inheriting from it directly is correct; flag any advice built on the pre-7.0 interface or its removed `DependencyObjectGenerator`. Logic that belongs in the shared layer should not be duplicated per-platform, and vice-versa.
...
- **Tech debt signals:** New `#pragma warning disable` without justification; project-wide analyzer suppression; `_Mux`-suffixed ported members (use natural WinUI names); a package `Version`/literal `VersionOverride` in a project file instead of `src/Directory.Packages.props` (CPM is on; DevServer add-in test fixtures are the only exception). *Patterns* that undermine async discipline (a new sync-over-async bridge, a new `async void` entry point outside event handlers) — leave per-call-site hunts to the skeptic/performance agents.
