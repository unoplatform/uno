...
- **DependencyObject generation** (`DependencyObject/`): `DependencyObject` is now a concrete base **class** that carries the property store and binder directly (the store was folded in — there is no separate `DependencyObjectStore` type or `IDependencyObjectStoreProvider` mixin). The generator no longer emits the store/`GetValue`/`SetValue` machinery; it only stamps per-type bits that must stay per-type (property registrations, `[Bindable]` on generated types).
