...
- **Folders are a nested namespace hierarchy** (like `src/Uno.UI`): `Windows/UI/Xaml/Controls/Button/`, `Microsoft/UI/Xaml/Controls/NavigationViewTests/`. The C# namespaces still use the legacy underscore form (`UITests.Shared.Windows_UI_Xaml_Controls.Button`) — copy the namespace from neighbouring files in the folder, don't derive it from the path.
- **`x:Class` must match the fully-qualified code-behind class exactly** (case-sensitive). File naming is PascalCase `Control_Scenario.xaml`. Co-locate supporting `.cs` (ViewModels) in the same folder.
...
- **Format before pushing**: `dotnet tool restore` then `dotnet xstyler -d src/SamplesApp -r` (single file: `-f <path>`; check-only: add `-p`) (config `Settings.XamlStyler`: one attribute per line, attribute reordering on). CI (`xaml-style-check.yml`) fails the PR on drift.
