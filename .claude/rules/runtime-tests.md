...
- **Folder mirrors the WinUI namespace**: a `Button` test goes in `Microsoft/UI/Xaml/Controls/Given_Button.cs` (pure WinRT areas stay under `Windows/`, MUX ports get a `.mux.cs` suffix). The C# namespace keeps the legacy form, e.g. `Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls`. Class `Given_<Control>`, method `When_<Scenario>`.
