...
1. **Entire implementation is platform-specific, in a per-platform project** → separate partial **file suffix**: `.Android.cs`, `.iOS.cs`, `.UIKit.cs` (iOS+tvOS), `.wasm.cs`, `.skia.cs`, `.reference.cs`. One file = exactly one platform/runtime; the suffix is auto-excluded elsewhere. Under `Uno.UI` only `.skia.cs` and `.crossruntime.cs` are ever compiled.
...
## Traps
- `#if __ANDROID__` is **false** everywhere in `Uno.UI` and in any `.skia.cs` file: that code compiles for plain `netX.0` and runs on Android unchanged. Use `OperatingSystem.IsAndroid()` for Android-only behavior there, or `#if __SKIA__` to separate it from other variants.
