...
- **TFMs come from `Directory.Build.props`** properties (`NetCurrent`, `NetPrevious`, and the `Net*PreviousAndCurrent` families — read that file for the values on your branch). Reference those properties — never hardcode a literal TFM such as `net11.0` in a `.csproj`.
