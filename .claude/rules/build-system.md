...
- **Central Package Management is enabled** for everything under `src/` except `src/SolutionTemplate/`. Every version lives in `src/Directory.Packages.props`; a `PackageReference` never carries `Version` (NU1008 fails the build). A project that must diverge sets `VersionOverride` to a named property defined in `Directory.Packages.props` (e.g. `$(RoslynGeneratorFloorVersion)`), never a literal; DevServer add-in test fixtures are the one exception, since their literal versions are the test data. `PackageReference Update` in `src/Directory.Build.targets` only adds asset metadata such as `ExcludeAssets`.
...
- **`RuntimeAssetsSelectorTask`** selects the `uno-runtime/<tfm>/<flavor>` assets for `Uno.WinRT`/`Uno.Foundation` and redirects the WinRT trio (`Uno.WinRT`, `Uno.Foundation`, `Uno.UI.Dispatching`) to the implementation matching the head's platform — don't override it manually. It no longer rewrites third-party packages: a multi-targeted library keeps its `lib/<tfm>-android|ios|tvos` asset.
  - It is driven by **`TargetPlatformIdentifier`**, not by a runtime identifier property; `UnoRuntimeIdentifier` / `UnoUIRuntimeIdentifier` / `UnoWinRTRuntimeIdentifier` no longer affect a head (UNOB0024). Whether replacement runs at all is **`UnoHasRuntimeHost`**, set by the `Uno.WinUI.Runtime.Skia.*` packages and by `Uno.Common.Desktop.targets`.
  - `<flavor>` is `skia` or `webassembly` — a **frozen package-layout name**, not a `RuntimeIdentifier` and not a drawing backend. It lives in the task as a constant; changing it silently breaks packages already on nuget.org, because a folder miss is not an error.
